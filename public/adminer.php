...
}function number($X)
{
    return
preg_replace('~[^0-9]+~', '', $X);
}function remove_slashes($Hf, $Jc=false)
{
    if (get_magic_quotes_gpc()) {
        while (list($x, $X)=each($Hf)) {
            foreach ($X
as$Gd=>$W) {
                unset($Hf[$x][$Gd]);
                if (is_array($W)) {
                    $Hf[$x][stripslashes($Gd)]=$W;
                    $Hf[]=&$Hf[$x][stripslashes($Gd)];
                } else {
                    $Hf[$x][stripslashes($Gd)]=($Jc?$W:stripslashes($W));
                }
            }
        }
    }
}function bracket_escape($t, $Na=false)
{
...
}function nl_br($P)
{
    return
str_replace("\n", "<br>", $P);
}function checkbox($C, $Y, $db, $Nd="", $Oe="", $ib="")
{
    $J="<input type='checkbox' name='$C' value='".h($Y)."'".($db?" checked":"").($Oe?' onclick="'.h($Oe).'"':'').">";
    return($Nd!=""||$ib?"<label".($ib?" class='$ib'":"").">$J".h($Nd)."</label>":$J);
}function optionlist($Ue, $sg=null, $Oh=false)
{
    $J="";
    foreach ($Ue
as$Gd=>$W) {
        $Ve=array($Gd=>$W); if (is_array($W)) {
            $J.='<optgroup label="'.h($Gd).'">';
            $Ve=$W;
        } foreach ($Ve
as$x=>$X) {
            $J.='<option'.($Oh||is_string($x)?' value="'.h($x).'"':'').(($Oh||is_string($x)?(string)$x:$X)===$sg?' selected':'').'>'.h($X);
        } if (is_array($W)) {
            $J.='</optgroup>';
        }
    }
    return$J;
}function html_select($C, $Ue, $Y="", $Ne=true)
{
    if ($Ne) {
        return"<select name='".h($C)."'".(is_string($Ne)?' onchange="'.h($Ne).'"':"").">".optionlist($Ue, $Y)."</select>";
    }
    $J="";
    foreach ($Ue
as$x=>$X) {
    ...
    return$J;
}function select_input($Ja, $Ue, $Y="", $uf="")
{
    return($Ue?"<select$Ja><option value=''>$uf".optionlist($Ue, $Y, true)."</select>":"<input$Ja size='10' value='".h($Y)."' placeholder='$uf'>");
}function confirm()
{
    return" onclick=\"return confirm('".lang(0)."');\"";
}function print_fieldset($jd, $Vd, $Zh=false, $Oe="")
{
    echo"<fieldset><legend><a href='#fieldset-$jd' onclick=\"".h($Oe)."return !toggle('fieldset-$jd');\">$Vd</a></legend><div id='fieldset-$jd'".($Zh?"":" class='hidden'").">\n";
}function bold($Va, $ib="")
{
...
}function sid()
{
    ...
    return$J;
}function set_password($Vh, $N, $V, $G)
{
...
}function get_vals($H, $e=0)
{
    ...
    return$J;
}function get_key_vals($H, $i=null, $jh=0)
{
    global$h;
    if (!is_object($i)) {
        $i=$h;
    ...
    return$J;
}function get_rows($H, $i=null, $n="<p class='error'>")
{
    global$h;
    $ub=(is_object($i)?$i:$h);
    $J=array();
    ...
    } elseif (!$I&&!is_object($i)&&$n&&defined("PAGE_HEADER")) {
        echo$n.error()."\n";
    }
    return$J;
...
}function where($Z, $p=array())
{
    ...
    foreach ((array)$Z["where"]as$x=>$X) {
        $x=bracket_escape($x, 1);$e=escape_key($x);$J[]=$e.(($w=="sql"&&preg_match('~^[0-9]*\\.[0-9]*$~', $X))||$w=="mssql"?" LIKE ".q(addcslashes($X, "%_\\")):" = ".unconvert_field($p[$x], q($X))); if ($w=="sql"&&preg_match('~char|text~', $p[$x]["type"])&&preg_match("~[^ -@]~", $X)) {
            $J[]="$e = ".q($X)." COLLATE ".charset($h)."_bin";
        }
    }
...
}function where_check($X, $p=array())
{
    ...
    return
where($bb, $p);
}function where_link($s, $e, $Y, $Qe="=")
{
    return"&where%5B$s%5D%5Bcol%5D=".urlencode($e)."&where%5B$s%5D%5Bop%5D=".urlencode(($Y!==null?$Qe:"IS NULL"))."&where%5B$s%5D%5Bval%5D=".urlencode($Y);
...
as$x=>$X) {
        if ($M&&!in_array(idf_escape($x), $M)) {
            continue;
        }$Ga=convert_field($p[$x]); if ($Ga) {
            $J.=", $Ga AS ".idf_escape($x);
        }
    }
    return$J;
}function cookie($C, $Y, $Xd=2592000)
{
    global$ba;
...
}function auth_url($Vh, $N, $V, $m=null)
{
    global$Xb;
...
}function redirect($A, $le=null)
{
    if ($le!==null) {
        restart_session();
        $_SESSION["messages"][preg_replace('~^[^?]*~', '', ($A!==null?$A:$_SERVER["REQUEST_URI"]))][]=$le;
    } if ($A!==null) {
        if ($A=="") {
            $A=".";
        }
...
}function query_redirect($H, $A, $le, $Rf=true, $wc=true, $Dc=false, $ih="")
{
    global$h,$n,$b;
    if ($wc) {
        $Hg=microtime(true);
...
array(implode("\n", $Lf),format_time($Hg));
    ...
    $Lf[]=(preg_match('~;$~', $H)?"DELIMITER ;;\n$H;\nDELIMITER ":$H).";";
    return$h->query($H);
}function apply_queries($H, $S, $sc='table')
{
    foreach ($S
as$Q) {
        if (!queries("$H ".$sc($Q))) {
...
false;
    ...
    return
true;
}function queries_redirect($A, $le, $Rf)
{
    list($Lf, $ih)=queries(null);
    return
query_redirect($Lf, $A, $le, $Rf, false, !$Rf, $ih);
}function format_time($Hg)
{
    return
lang(1, max(0, microtime(true)-$Hg));
}function remove_from_uri($if="")
{
    return
substr(preg_replace("~(?<=[?&])($if".(SID?"":"|".session_name()).")=[^&]*&~", '', "$_SERVER[REQUEST_URI]&"), 0, -1);
}function pagination($E, $Db)
{
    return" ".($E==$Db?$E+1:'<a href="'.h(remove_from_uri("page").($E?"&page=$E".($_GET["next"]?"&next=".urlencode($_GET["next"]):""):"")).'">'.($E+1)."</a>");
}function get_file($x, $Kb=false)
{
    $Hc=$_FILES[$x];
    if (!$Hc) {
        return
...
as$x=>$X) {
    ...
    foreach ($Hc["error"]as$x=>$n) {
        if ($n) {
            return$n;
        }$C=$Hc["name"][$x];$qh=$Hc["tmp_name"][$x];$wb=file_get_contents($Kb&&preg_match('~\\.gz$~', $C)?"compress.zlib://$qh":$qh); if ($Kb) {
            $Hg=substr($wb, 0, 3);
            if (function_exists("iconv")&&preg_match("~^\xFE\xFF|^\xFF\xFE~", $Hg, $Xf)) {
                $wb=iconv("utf-16", "utf-8", $wb);
...
}function upload_error($n)
{
    ...
    return($n?lang(2).($ie?" ".lang(3, $ie):""):lang(4));
}function repeat_pattern($sf, $y)
{
    return
str_repeat("$sf{0,65535}", $y/65535)."$sf{0,".($y%65535)."}";
}function is_utf8($X)
{
    return(preg_match('~~u', $X)&&!preg_match('~[\\0-\\x8\\xB\\xC\\xE-\\x1F]~', $X));
}function shorten_utf8($P, $y=80, $Og="")
{
...
}function hidden_fields($Hf, $md=array())
{
    while (list($x, $X)=each($Hf)) {
        if (!in_array($x, $md)) {
            if (is_array($X)) {
...
}function table_status1($Q, $Ec=false)
{
    $J=table_status($Q, $Ec);
...
}function column_foreign_keys($Q)
{
    ...
    return$J;
}function enum_input($U, $Ja, $o, $Y, $mc=null)
{
    global$b;
...
{
    global$h,$Bh,$b,$w;$C=h(bracket_escape($o["field"]));echo"<td class='function'>"; if (is_array($Y)&&!$r) {
        $Ea=array($Y);
        ...
        $r="json";
    }$ag=($w=="mssql"&&$o["auto_increment"]); if ($ag&&!$_POST["save"]) {
        $r=null;
    }$Uc=(isset($_GET["select"])||$ag?array("orig"=>lang(8)):array())+$b->editFunctions($o);$Ja=" name='fields[$C]'"; if ($o["type"]=="enum") {
        echo
nbsp($Uc[""])."<td>".$b->editInput($_GET["edit"], $o, $Ja, $Y);
    } else {
        $Kc=0;
        foreach ($Uc
as$x=>$X) {
        ...
        $Ne=($Kc?" onchange=\"var f = this.form['function[".h(js_escape(bracket_escape($o["field"])))."]']; if ($Kc > f.selectedIndex) f.selectedIndex = $Kc;\" onkeyup='keyupChange.call(this);'":"");
        $Ja.=$Ne;
        ...
        if ($vd!="") {
            ...
            preg_match_all("~'((?:[^']|'')*)'~", $o["length"], $de); foreach ($de[1]as$s=>$X) {
                $X=stripcslashes(str_replace("''", "'", $X));
                $db=(is_int($Y)?($Y>>$s)&1:in_array($X, explode(",", $Y), true));
                echo" <label><input type='checkbox' name='fields[$C][$s]' value='".(1<<$s)."'".($db?' checked':'')."$Ne>".h($b->editVal($X, $o)).'</label>';
            }
        } elseif (preg_match('~blob|bytea|raw|file~', $o["type"])&&ini_bool("file_uploads")) {
            echo"<input type='file' name='fields-$C'$Ne>";
            ...
            $ke=(!preg_match('~int~', $o["type"])&&preg_match('~^(\\d+)(,(\\d+))?$~', $o["length"], $B)?((preg_match("~binary~", $o["type"])?2:1)*$B[1]+($B[3]?1:0)+($B[2]&&!$o["unsigned"]?1:0)):($Bh[$o["type"]]?$Bh[$o["type"]]+($o["unsigned"]?0:1):0));
            if ($w=='sql'&&$h->server_info>=5.6&&preg_match('~time~', $o["type"])) {
                $ke+=7;
            }
            echo"<input".((!$cd||$r==="")&&preg_match('~(?<!o)int~', $o["type"])?" type='number'":"")." value='".h($Y)."'".($ke?" maxlength='$ke'":"").(preg_match('~char|binary~', $o["type"])&&$ke>20?" size='40'":"")."$Ja>";
        }
    }
...
}function search_tables()
{
    ...
    foreach (table_status('', true)as$Q=>$R) {
        $C=$b->tableName($R); if (isset($R["Engine"])&&$C!=""&&(!$_POST["tables"]||in_array($Q, $_POST["tables"]))) {
...
}function dump_headers($kd, $ue=false)
{
    global$b;
    $J=$b->dumpHeaders($kd, $ue);
    $gf=$_POST["output"];
    if ($gf!="text") {
        header("Content-Disposition: attachment; filename=".$b->dumpFilename($kd).".$J".($gf!="file"&&!preg_match('~[^0-9a-z]~', $gf)?".$gf":""));
    }
    session_write_close();
    ob_flush();
    flush();
...
}function select_value($X, $_, $o, $hh)
{
    global$b,$ba;
    if (is_array($X)) {
        $J="";
        foreach ($X
as$Gd=>$W) {
            $J.="<tr>".($X!=array_values($X)?"<th>".h($Gd):"")."<td>".select_value($W, $_, $o, $hh);
        }
    ...
    if ($_===null) {
        if (is_mail($X)) {
            ...
            $_=(($Jf=="http"&&$ba)||preg_match('~WebKit~i', $_SERVER["HTTP_USER_AGENT"])?$X:"https://www.adminer.org/redirect/?url=".urlencode($X));
        }
    }
    $J=$b->editVal($X, $o);
    if ($J!==null) {
        if ($J==="") {
            ...
            $J="\0";
        } elseif ($hh!=""&&is_shortable($o)) {
            $J=shorten_utf8($J, max(0, +$hh));
        } else {
            $J=h($J);
        }
    }
...
}function is_mail($jc)
{
    ...
    $sf="$Ha+(\\.$Ha+)*@($Vb?\\.)+$Vb";
    return
is_string($jc)&&preg_match("(^$sf(,\\s*$sf)*\$)i", $jc);
}function is_url($P)
{
    $Vb='[a-z0-9]([-a-z0-9]{0,61}[a-z0-9])';
...
{
    $Of=rand(1, 1e6);
    return($Of^$_SESSION["token"]).":$Of";
}function verify_token()
{
    list($T, $Of)=explode(":", $_POST["token"]);
    return($Of^$_SESSION["token"])==$T;
}function lzw_decompress($Ra)
{
    ...
    for ($s=0;$s<strlen($Ra);$s++) {
        $cg=($cg<<8)+ord($Ra[$s]);$dg+=8; if ($dg>=$Sa) {
            $dg-=$Sa;
            $kb[]=$cg>>$dg;
            $cg&=(1<<$dg)-1;
            $Rb++;
...
as$s=>$jb) {
    ...
    return$J;
}function on_help($pb, $_g=0)
{
    return" onmouseover='helpMouseover(this, event, ".h($pb).", $_g);' onmouseout='helpMouseout(this, event);'";
}function edit_form($a, $p, $K, $Jh)
{
    global$b,$w,$T,$n;
...
as$C=>$o) {
            ...
            if ($Lb===null) {
                $Lb=$o["default"]; if ($o["type"]=="bit"&&preg_match("~^b'([01]*)'\$~", $Lb, $Xf)) {
                    $Lb=$Xf[1];
                }
            }
    ...
    if ($p) {
        echo"<input type='submit' value='".lang(14)."'>\n"; if (!isset($_GET["select"])) {
            echo"<input type='submit' name='insert' value='".($Jh?lang(15)."' onclick='return !ajaxForm(this.form, \"".lang(16).'...", this)':lang(17))."' title='Ctrl+Shift+Enter'>\n";
        }
    }
...
';
}global$b,$h,$Xb,$fc,$pc,$n,$Uc,$Zc,$ba,$ud,$w,$ca,$Pd,$Me,$tf,$Lg,$dd,$T,$vh,$Bh,$Ih,$ia; if (!$_SERVER["REQUEST_URI"]) {
    ...
    session_start();
}remove_slashes(array(&$_GET,&$_POST,&$_COOKIE), $Jc); if (get_magic_quotes_runtime()) {
...
}@set_time_limit(0);@ini_set("zend.ze1_compatibility_mode", false);@ini_set("precision", 20);$Pd=array('en'=>'English','ar'=>'Ø§Ù„Ø¹Ø±Ø¨ÙŠØ©','bg'=>'Ð‘ÑŠÐ»Ð³Ð°Ñ€ÑÐºÐ¸','bn'=>'à¦¬à¦¾à¦‚à¦²à¦¾','bs'=>'Bosanski','ca'=>'CatalÃ ','cs'=>'ÄŒeÅ¡tina','da'=>'Dansk','de'=>'Deutsch','el'=>'Î•Î»Î»Î·Î½Î¹ÎºÎ¬','es'=>'EspaÃ±ol','et'=>'Eesti','fa'=>'ÙØ§Ø±Ø³ÛŒ','fi'=>'Suomi','fr'=>'FranÃ§ais','gl'=>'Galego','hu'=>'Magyar','id'=>'Bahasa Indonesia','it'=>'Italiano','ja'=>'æ—¥æœ¬èªž','ko'=>'í•œêµ­ì–´','lt'=>'LietuviÅ³','nl'=>'Nederlands','no'=>'Norsk','pl'=>'Polski','pt'=>'PortuguÃªs','pt-br'=>'PortuguÃªs (Brazil)','ro'=>'Limba RomÃ¢nÄƒ','ru'=>'Ð ÑƒÑÑÐºÐ¸Ð¹ ÑÐ·Ñ‹Ðº','sk'=>'SlovenÄina','sl'=>'Slovenski','sr'=>'Ð¡Ñ€Ð¿ÑÐºÐ¸','ta'=>'à®¤â€Œà®®à®¿à®´à¯','th'=>'à¸ à¸²à¸©à¸²à¹„à¸—à¸¢','tr'=>'TÃ¼rkÃ§e','uk'=>'Ð£ÐºÑ€Ð°Ñ—Ð½ÑÑŒÐºÐ°','vi'=>'Tiáº¿ng Viá»‡t','zh'=>'ç®€ä½“ä¸­æ–‡','zh-tw'=>'ç¹é«”ä¸­æ–‡',);function get_lang()
{
    global$ca;
    return$ca;
}function lang($t, $De=null)
{
    if (is_string($t)) {
        $wf=array_search($t, get_translations("en")); if ($wf!==false) {
            $t=$wf;
        }
    }
    global$ca,$vh;
    $uh=($vh[$t]?$vh[$t]:$t);
    if (is_array($uh)) {
        $wf=($De==1?0:($ca=='cs'||$ca=='sk'?($De&&$De<5?1:2):($ca=='fr'?(!$De?0:1):($ca=='pl'?($De%10>1&&$De%10<5&&$De/10%10!=1?1:2):($ca=='sl'?($De%100==1?0:($De%100==2?1:($De%100==3||$De%100==4?2:3))):($ca=='lt'?($De%10==1&&$De%100!=11?0:($De%10>1&&$De/10%10!=1?1:2)):($ca=='bs'||$ca=='ru'||$ca=='sr'||$ca=='uk'?($De%10==1&&$De%100!=11?0:($De%10>1&&$De%10<5&&$De/10%10!=1?1:2)):1)))))));
        $uh=$uh[$wf];
    }
    $Ea=func_get_args();
    array_shift($Ea);
...
}$ca="en"; if (isset($Pd[$_COOKIE["adminer_lang"]])) {
    cookie("adminer_lang", $_COOKIE["adminer_lang"]);
    $ca=$_COOKIE["adminer_lang"];
} elseif (isset($Pd[$_SESSION["lang"]])) {
    $ca=$_SESSION["lang"];
} else {
    $ua=array();preg_match_all('~([-a-z]+)(;q=([0-9.]+))?~', str_replace("_", "-", strtolower($_SERVER["HTTP_ACCEPT_LANGUAGE"])), $de, PREG_SET_ORDER); foreach ($de
as$B) {
        $ua[$B[1]]=(isset($B[3])?$B[3]:1);
    }arsort($ua); foreach ($ua
as$x=>$Kf) {
        if (isset($Pd[$x])) {
            $ca=$x;
            break;
...
PDO
    {
        ...
        {
            global$b;$wf=array_search("SQL", $b->operators); if ($wf!==false) {
                unset($b->operators[$wf]);
            }
        }
        public function dsn($cc, $V, $G)
        {
            try {
        ...
        public function query($H, $Ch=false)
        {
            $I=parent::query($H);
            $this->error="";
            if (!$I) {
...
}$Xb=array();class Min_SQL
{
    public $_conn;public function __construct($h)
    {
        $this->_conn=$h;
    }public function select($Q, $M, $Z, $Xc, $We=array(), $z=1, $E=0, $Df=false)
    {
        global$b,$w;
        $Ad=(count($Xc)<count($M));
        $H=$b->selectQueryBuild($M, $Z, $Xc, $We, $z, $E);
        if (!$H) {
            $H="SELECT".limit(($_GET["page"]!="last"&&+$z&&$Xc&&$Ad&&$w=="sql"?"SQL_CALC_FOUND_ROWS ":"").implode(", ", $M)."\nFROM ".table($Q), ($Z?"\nWHERE ".implode(" AND ", $Z):"").($Xc&&$Ad?"\nGROUP BY ".implode(", ", $Xc):"").($We?"\nORDER BY ".implode(", ", $We):""), ($z!=""?+$z:null), ($E?$z*$E:0), "\n");
        }
        $Hg=microtime(true);
        $J=$this->_conn->query($H);
        if ($Df) {
            echo$b->selectQuery($H, format_time($Hg));
        }
        return$J;
    }public function delete($Q, $Mf, $z=0)
    {
        $H="FROM ".table($Q);
        return
queries("DELETE".($z?limit1($H, $Mf):" $H$Mf"));
    }public function update($Q, $O, $Mf, $z=0, $ug="\n")
    {
        $Th=array();
        foreach ($O
as$x=>$X) {
        ...
        $H=table($Q)." SET$ug".implode(",$ug", $Th);
        return
queries("UPDATE".($z?limit1($H, $Mf):" $H$Mf"));
    }public function insert($Q, $O)
    {
        return
queries("INSERT INTO ".table($Q).($O?" (".implode(", ", array_keys($O)).")\nVALUES (".implode(", ", $O).")":" DEFAULT VALUES"));
    }public function insertUpdate($Q, $L, $Bf)
    {
        return
false;
...
}$Xb["sqlite"]="SQLite 3";$Xb["sqlite2"]="SQLite 2"; if (isset($_GET["sqlite"])||isset($_GET["sqlite2"])) {
    $zf=array((isset($_GET["sqlite"])?"SQLite3":"SQLite"),"PDO_SQLite");
    define("DRIVER", (isset($_GET["sqlite"])?"sqlite":"sqlite2"));
    if (class_exists(isset($_GET["sqlite"])?"SQLite3":"SQLiteDatabase")) {
        if (isset($_GET["sqlite"])) {
            class Min_SQLite
            {
                public $extension="SQLite3";public $server_infopublic $affected_rowspublic $errnopublic $errorpublic $_linkvar ;public function __construct($Ic)
                {
                    $this->_link=new
SQLite3($Ic);
...
false;
            ...
            class Min_Result
            {
                public $_result;public $_offsetvar =0;public $num_rowsvar ;public function __construct($I)
                {
                    $this->_result=$I;
                }public function fetch_assoc()
                {
            ...
            class Min_SQLite
            {
                public $extension="SQLite";public $server_infopublic $affected_rowspublic $errorpublic $_linkvar ;public function __construct($Ic)
                {
...
SQLiteDatabase($Ic);
                ...
                {
                    $re=($Ch?"unbufferedQuery":"query");
                    $I=@$this->_link->$re($H, SQLITE_BOTH, $n);
                    $this->error="";
                    if (!$I) {
                        $this->error=$n;
                        return
...
array();
    ...
    function limit($H, $Z, $z, $D=0, $ug=" ")
    {
        return" $H$Z".($z!==null?$ug."LIMIT $z".($D?" OFFSET $D":""):"");
    }
    function limit1($H, $Z)
    {
        global$h;
...
array();
    ...
    function fields($Q)
    {
        ...
        preg_match_all('~(("[^"]*+")+|[a-z0-9_]+)\s+text\s+COLLATE\s+(\'[^\']+\'|\S+)~i', $Fg, $de, PREG_SET_ORDER);
        foreach ($de
as$B) {
            $C=str_replace('""', '"', preg_replace('~^"|"$~', '', $B[1])); if ($J[$C]) {
    ...
    function indexes($Q, $i=null)
    {
        ...
        if (preg_match('~\bPRIMARY\s+KEY\s*\((([^)"]+|"[^"]*")++)~i', $Fg, $B)) {
            $J[""]=array("type"=>"PRIMARY","columns"=>array(),"lengths"=>array(),"descs"=>array());preg_match_all('~((("[^"]*+")+)|(\S+))(\s+(ASC|DESC))?(,\s*|$)~i', $B[1], $de, PREG_SET_ORDER); foreach ($de
as$B) {
        ...
        if (!$J) {
            foreach (fields($Q)as$C=>$o) {
                if ($o["primary"]) {
                    $J[""]=array("type"=>"PRIMARY","columns"=>array($C),"lengths"=>array(),"descs"=>array(null));
                }
            }
        }
        ...
        foreach (get_rows("PRAGMA index_list(".table($Q).")", $i)as$K) {
            $C=$K["name"];$u=array("type"=>($K["unique"]?"UNIQUE":"INDEX"));$u["lengths"]=array();$u["descs"]=array(); foreach (get_rows("PRAGMA index_info(".idf_escape($C).")", $i)as$kg) {
                $u["columns"][]=$kg["name"];
                $u["descs"][]=null;
            } if (preg_match('~^CREATE( UNIQUE)? INDEX '.preg_quote(idf_escape($C).' ON '.idf_escape($Q), '~').' \((.*)\)$~i', $Gg[$C], $Xf)) {
                preg_match_all('/("[^"]*+")+( DESC)?/', $Xf[2], $de);
                foreach ($de[2]as$x=>$X) {
                    if ($X) {
                        $u["descs"][$x]='1';
                    }
...
false;
    ...
    function alter_table($Q, $C, $p, $Mc, $rb, $nc, $d, $La, $nf)
    {
        $Nh=($Q==""||$Mc);
        foreach ($p
as$o) {
...
false;
        ...
        } elseif (!recreate_table($Q, $C, $c, $ef, $Mc)) {
            return
false;
        }
        if ($La) {
...
true;
    ...
    function recreate_table($Q, $C, $p, $ef, $Mc, $v=array())
    {
        if ($Q!="") {
            if (!$p) {
...
as$x=>$X) {
            ...
            foreach (indexes($Q)as$Jd=>$u) {
                $f=array(); foreach ($u["columns"]as$x=>$e) {
                    if (!$ef[$e]) {
                        continue
2;
                    }
                    $f[]=$ef[$e].($u["descs"][$x]?" DESC":"");
                } if (!$ac[$Jd]) {
                    if ($u["type"]!="PRIMARY"||!$Cf) {
                        $v[]=array($u["type"],$Jd,$f);
                    }
                }
            }
...
2;
                    ...
                    $q["source"][$x]=idf_unescape($ef[$e]);
                } if (!isset($Mc[" $Jd"])) {
                    $Mc[]=" ".format_foreign_key($q);
                }
            }
            queries("BEGIN");
        }
...
false;
        ...
        if ($Q!="") {
            if ($ef&&!queries("INSERT INTO ".table("adminer_$C")." (".implode(", ", $ef).") SELECT ".implode(", ", array_map('idf_escape', array_keys($ef)))." FROM ".table($Q))) {
                return
false;
            }
            $zh=array();
...
array("Statement"=>"BEGIN\n\t;\nEND");
        ...
        $Fe=$B[3];
        return
array("Timing"=>strtoupper($B[1]),"Event"=>strtoupper($B[2]).($Fe?" OF":""),"Of"=>($Fe[0]=='`'||$Fe[0]=='"'?idf_unescape($Fe):$Fe),"Trigger"=>$C,"Statement"=>$B[4],);
    }
    function triggers($Q)
    {
        $J=array();
...
array();
    ...
    function set_schema($og)
    {
        return
true;
    }
    function create_sql($Q, $La)
    {
        global$h;
...
preg_match('~^(columns|database|drop_col|dump|indexes|move_col|sql|status|table|trigger|variables|view|view_trigger)$~', $Fc);
    ...
    $Lg=array_keys($Bh);
    $Ih=array();
    $Re=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT IN","IS NOT NULL","SQL");
...
}$Xb["pgsql"]="PostgreSQL"; if (isset($_GET["pgsql"])) {
    $zf=array("PgSQL","PDO_PgSQL");
    define("DRIVER", "pgsql");
    if (extension_loaded("pgsql")) {
        class Min_DB
        {
            public $extension="PgSQL";
            public $_linkpublic $_resultpublic $_stringpublic $_databasevar =true;
            public $server_infopublic $affected_rowspublic $errorvar ;
            public function _error($qc, $n)
            {
...
Min_SQL
    {
        public function insertUpdate($Q, $L, $Bf)
        {
            ...
            foreach ($L
as$O) {
                $Jh=array();$Z=array(); foreach ($O
as$x=>$X) {
                    $Jh[]="$x = $X";
                    if (isset($Bf[idf_unescape($x)])) {
                        $Z[]="$x = $X";
                    }
                } if (!(($Z&&queries("UPDATE ".table($Q)." SET ".implode(", ", $Jh)." WHERE ".implode(" AND ", $Z))&&$h->affected_rows)||queries("INSERT INTO ".table($Q)." (".implode(", ", array_keys($O)).") VALUES (".implode(", ", $O).")"))) {
                    return
false;
                }
            }
...
ORDER BY a.attnum")as$K) {
    ...
    function indexes($Q, $i=null)
    {
        ...
        foreach (get_rows("SELECT relname, indisunique::int, indisprimary::int, indkey, indoption FROM pg_index i, pg_class ci WHERE i.indrelid = $Ug AND ci.oid = i.indexrelid", $i)as$K) {
            $Yf=$K["relname"];
            $J[$Yf]["type"]=($K["indisprimary"]?"PRIMARY":($K["indisunique"]?"UNIQUE":"INDEX"));
            $J[$Yf]["columns"]=array();
            foreach (explode(" ", $K["indkey"])as$qd) {
                $J[$Yf]["columns"][]=$f[$qd];
            }
            $J[$Yf]["descs"]=array();
            foreach (explode(" ", $K["indoption"])as$rd) {
                $J[$Yf]["descs"][]=($rd&1?'1':null);
            }
            $J[$Yf]["lengths"]=array();
        }
        return$J;
...
ORDER BY conkey, conname")as$K) {
            if (preg_match('~FOREIGN KEY\s*\((.+)\)\s*REFERENCES (.+)\((.+)\)(.*)$~iA', $K['definition'], $B)) {
                ...
                if (preg_match('~^(("([^"]|"")+"|[^"]+)\.)?"?("([^"]|"")+"|[^"]+)$~', $B[2], $ce)) {
                    $K['ns']=str_replace('""', '"', preg_replace('~^"(.+)"$~', '\1', $ce[2]));
                    $K['table']=str_replace('""', '"', preg_replace('~^"(.+)"$~', '\1', $ce[4]));
                }
                $K['target']=array_map('trim', explode(',', $B[3]));
                $K['on_delete']=(preg_match("~ON DELETE ($Me)~", $B[4], $ce)?$ce[1]:'NO ACTION');
                $K['on_update']=(preg_match("~ON UPDATE ($Me)~", $B[4], $ce)?$ce[1]:'NO ACTION');
                $J[$K['conname']]=$K;
            }
        }
        return$J;
...
as$o) {
            $e=idf_escape($o[0]);$X=$o[1]; if (!$X) {
                ...
                if ($o[0]=="") {
                    ...
                    if ($e!=$X[0]) {
                        $Lf[]="ALTER TABLE ".table($Q)." RENAME $e TO $X[0]";
                    }$c[]="ALTER $e TYPE$X[1]"; if (!$X[6]) {
                        $c[]="ALTER $e ".($X[3]?"SET$X[3]":"DROP DEFAULT");
                        $c[]="ALTER $e ".($X[2]==" NULL"?"DROP NOT":"SET").$X[2];
                    }
                }
                if ($o[0]!=""||$Sh!="") {
                    $Lf[]="COMMENT ON COLUMN ".table($Q).".$X[0] IS ".($Sh!=""?substr($Sh, 9):"''");
                }
            }
        }
        $c=array_merge($c, $Mc);
        if ($Q=="") {
            array_unshift($Lf, "CREATE TABLE ".table($C)." (\n".implode(",\n", $c)."\n)");
        } elseif ($c) {
            array_unshift($Lf, "ALTER TABLE ".table($Q)."\n".implode(",\n", $c));
        }
        if ($Q!=""&&$Q!=$C) {
            $Lf[]="ALTER TABLE ".table($Q)." RENAME TO ".table($C);
        }
        if ($Q!=""||$rb!="") {
            $Lf[]="COMMENT ON TABLE ".table($C)." IS ".q($rb);
        }
        if ($La!="") {
        }
        foreach ($Lf
...
as$X) {
            if ($X[0]!="INDEX") {
                ...
                $Lf[]="CREATE INDEX ".idf_escape($X[1]!=""?$X[1]:uniqid($Q."_"))." ON ".table($Q)." (".implode(", ", $X[2]).")";
            }
        }
        if ($j) {
            array_unshift($Lf, "ALTER TABLE ".table($Q).implode(",", $j));
        }
        if ($Yb) {
            array_unshift($Lf, "DROP INDEX ".implode(", ", $Yb));
        }
        foreach ($Lf
as$H) {
            if (!queries($H)) {
...
drop_tables($Yh);
    ...
    function drop_tables($S)
    {
        foreach ($S
as$Q) {
            $Ig=table_status($Q); if (!queries("DROP ".strtoupper($Ig["Engine"])." ".table($Q))) {
                return
false;
            }
        }
...
true;
    ...
    function move_tables($S, $Yh, $bh)
    {
        foreach (array_merge($S, $Yh)as$Q) {
            $Ig=table_status($Q); if (!queries("ALTER ".strtoupper($Ig["Engine"])." ".table($Q)." SET SCHEMA ".idf_escape($bh))) {
                return
false;
            }
        }
...
reset($L);
    ...
    function triggers($Q)
    {
        ...
        foreach (get_rows("SELECT * FROM information_schema.triggers WHERE event_object_table = ".q($Q))as$K) {
            $J[$K["trigger_name"]]=array($K["condition_timing"],$K["event_manipulation"]);
        }
        return$J;
    }
    function trigger_options()
...
0;
    ...
    {
        global$h;
        if (preg_match("~ rows=([0-9]+)~", $h->result("EXPLAIN SELECT * FROM ".idf_escape($R["Name"]).($Z?" WHERE ".implode(" AND ", $Z):"")), $Xf)) {
            return$Xf[1];
        }
        return
false;
...
get_vals("SELECT nspname FROM pg_namespace ORDER BY nspname");
    ...
    function set_schema($ng)
    {
        global$h,$Bh,$Lg;
        $J=$h->query("SET search_path TO ".idf_escape($ng));
        foreach (types()as$U) {
            if (!isset($Bh[$U])) {
                $Bh[$U]=0;
                $Lg[lang(23)][]=$U;
...
get_rows("SELECT * FROM pg_stat_activity ORDER BY ".($h->server_info<9.2?"procpid":"pid"));
    ...
    function support($Fc)
    {
        global$h;
        return
preg_match('~^(database|table|columns|sql|indexes|comment|view|'.($h->server_info>=9.3?'materializedview|':'').'scheme|processlist|sequence|trigger|type|variables|drop_col|kill)$~', $Fc);
    }
...
queries("SELECT pg_terminate_backend(".number($X).")");
    ...
    $Ih=array();
    $Re=array("=","<",">","<=",">=","!=","~","!~","LIKE","LIKE %%","ILIKE","ILIKE %%","IN","IS NULL","NOT LIKE","NOT IN","IS NOT NULL");
...
}$Xb["oracle"]="Oracle"; if (isset($_GET["oracle"])) {
    $zf=array("OCI8","PDO_OCI");
    define("DRIVER", "oracle");
    if (extension_loaded("oci8")) {
        class Min_DB
        {
            public $extension="oci8";
            public $_linkpublic $_resultpublic $server_infopublic $affected_rowspublic $errnopublic $errorvar ;
            public function _error($qc, $n)
            {
                if (ini_bool("html_errors")) {
                    $n=html_entity_decode(strip_tags($n));
...
array();
    ...
    function table_status($C="")
    {
        $J=array();
        $pg=q($C);
        foreach (get_rows('SELECT table_name "Name", \'table\' "Engine", avg_row_len * num_rows "Data_length", num_rows "Rows" FROM all_tables WHERE tablespace_name = '.q(DB).($C!=""?" AND table_name = $pg":"")."
...
h($h->error);
    ...
    function alter_table($Q, $C, $p, $Mc, $rb, $nc, $d, $La, $nf)
    {
        ...
        foreach ($p
as$o) {
            $X=$o[1]; if ($X&&$o[0]!=""&&idf_escape($o[0])!=$X[0]) {
...
AND c_src.TABLE_NAME = ".q($Q);
        foreach (get_rows($H)as$K) {
            $J[$K['NAME']]=array("db"=>$K['DEST_DB'],"table"=>$K['DEST_TABLE'],"source"=>array($K['SRC_COLUMN']),"target"=>array($K['DEST_COLUMN']),"on_delete"=>$K['ON_DELETE'],"on_update"=>null,);
        }
        return$J;
    }
    function truncate_tables($S)
...
get_vals("SELECT DISTINCT owner FROM dba_segments WHERE owner IN (SELECT username FROM dba_users WHERE default_tablespace NOT IN ('SYSTEM','SYSAUX'))");
    ...
    {
        global$h;
        return$h->query("ALTER SESSION SET CURRENT_SCHEMA = ".idf_escape($og));
    }
    function show_variables()
    {
        return
...
preg_match('~^(columns|database|drop_col|indexes|processlist|scheme|sql|status|table|variables|view|view_trigger)$~', $Fc);
    ...
    $Ih=array();
    $Re=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT REGEXP","NOT IN","IS NOT NULL","SQL");
...
}$Xb["mssql"]="MS SQL"; if (isset($_GET["mssql"])) {
    $zf=array("SQLSRV","MSSQL");
    define("DRIVER", "mssql");
...
false;
        ...
        }class Min_Result
        {
            public $_result;
            public $_offsetvar =0;
            public $_fieldspublic $num_rowsvar ;
            public function __construct($I)
            {
                $this->_result=$I;
            }
...
stdClass;
        ...
        class Min_DB
        {
            ...
            public $_linkpublic $_resultpublic $server_infopublic $affected_rowspublic $errorvar ;
            public function connect($N, $V, $G)
            {
...
as$x=>$X) {
                ...
                } if (!queries("MERGE ".table($Q)." USING (VALUES(".implode(", ", $O).")) AS source (c".implode(", c", range(1, count($O))).") ON ".implode(" AND ", $Z)." WHEN MATCHED THEN UPDATE SET ".implode(", ", $Jh)." WHEN NOT MATCHED THEN INSERT (".implode(", ", array_keys($O)).") VALUES (".implode(", ", $O).");")) {
                    return
false;
                }
            }
...
as$o) {
            $e=idf_escape($o[0]);$X=$o[1]; if (!$X) {
                ...
                if ($o[0]=="") {
                    $c["ADD"][]="\n  ".implode("", $X).($Q==""?substr($Mc[$X[0]], 16+strlen($X[0])):"");
                } else {
                    unset($X[6]);
                    if ($e!=$X[0]) {
...
array();
    ...
    function table_status($C="", $Ec=false)
    {
        global$h;
        $J=array();
        $Eb=tables_list();
        foreach ($Eb
as$u=>$X) {
            $u=trim($u);$J[$u]=array('Name'=>$u,'Engine'=>'standard',); if ($C==$u) {
                return$J[$u];
            }
        }
        return$J;
...
ORDER BY r.RDB$FIELD_POSITION';
        $I=ibase_query($h->_link, $H);
        while ($K=ibase_fetch_assoc($I)) {
            $J[trim($K['FIELD_NAME'])]=array("field"=>trim($K["FIELD_NAME"]),"full_type"=>trim($K["FIELD_TYPE"]),"type"=>trim($K["FIELD_SUB_TYPE"]),"default"=>trim($K['FIELD_DEFAULT_VALUE']),"null"=>(trim($K["FIELD_NOT_NULL_CONSTRAINT"])=="YES"),"auto_increment"=>'0',"collation"=>trim($K["FIELD_COLLATION"]),"privileges"=>array("insert"=>1,"select"=>1,"update"=>1),"comment"=>trim($K["FIELD_DESCRIPTION"]),);
        }
        return$J;
    }
    function indexes($Q, $i=null)
...
preg_match("~^(columns|sql|status|table)$~", $Fc);
    ...
    $Zc=array();
    $fc=array();
}$Xb["simpledb"]="SimpleDB"; if (isset($_GET["simpledb"])) {
    $zf=array("SimpleXML");
    ...
    if (class_exists('SimpleXMLElement')) {
        class Min_DB
        {
            ...
            public $errorpublic $timeoutpublic $nextpublic $affected_rowspublic $_resultvar ;
            public function select_db($Gb)
            {
                return($Gb=="domain");
            }
...
as$x=>$X) {
            ...
            public function _processValue($ic)
            {
...
Min_SQL
    {
        public $Bf="itemName()";public function _chunkRequest($ld, $va, $F, $yc=array())
        {
            global$h;
            foreach (array_chunk($ld, 25)as$gb) {
                $jf=$F; foreach ($gb
as$s=>$jd) {
                    $jf["Item.$s.ItemName"]=$jd;
                    foreach ($yc
as$x=>$X) {
                        $jf["Item.$s.$x"]=$X;
                    }
                } if (!sdb_request($va, $jf)) {
                    return
false;
                }
            }
            ...
            return
true;
        }public function _extractIds($Q, $Mf, $z)
        {
            $J=array();
            if (preg_match_all("~itemName\(\) = (('[^']*+')+)~", $Mf, $de)) {
                $J=array_map('idf_unescape', $de[1]);
            } else {
                foreach (sdb_request_all('Select', 'Item', array('SelectExpression'=>'SELECT itemName() FROM '.table($Q).$Mf.($z?" LIMIT 1":"")))as$Ed) {
                    $J[]=$Ed->Name;
                }
            }
            return$J;
        }public function select($Q, $M, $Z, $Xc, $We=array(), $z=1, $E=0, $Df=false)
        {
            global$h;
            $h->next=$_GET["next"];
            $J=parent::select($Q, $M, $Z, $Xc, $We, $z, $E, $Df);
            $h->next=0;
            return$J;
        }public function delete($Q, $Mf, $z=0)
        {
            return$this->_chunkRequest($this->_extractIds($Q, $Mf, $z), 'BatchDeleteAttributes', array('DomainName'=>$Q));
        }public function update($Q, $O, $Mf, $z=0, $ug="\n")
        {
            $Mb=array();
            $wd=array();
            $s=0;
            $ld=$this->_extractIds($Q, $Mf, $z);
            $jd=idf_unescape($O["`itemName()`"]);
            unset($O["`itemName()`"]);
            foreach ($O
as$x=>$X) {
                $x=idf_unescape($x); if ($X=="NULL"||($jd!=""&&array($jd)!=$ld)) {
                    $Mb["Attribute.".count($Mb).".Name"]=$x;
                } if ($X!="NULL") {
                    foreach ((array)$X
as$Gd=>$W) {
...
array();
    ...
    function table_status($C="", $Ec=false)
    {
        ...
        foreach (($C!=""?array($C=>true):tables_list())as$Q=>$U) {
            ...
            if (!$Ec) {
                $qe=sdb_request('DomainMetadata', array('DomainName'=>$Q)); if ($qe) {
                    foreach (array("Rows"=>"ItemCount","Data_length"=>"ItemNamesSizeBytes","Index_length"=>"AttributeValuesSizeBytes","Data_free"=>"AttributeNamesSizeBytes",)as$x=>$X) {
                        $K[$x]=(string)$qe->$X;
                    }
                }
            }
...
array();
    ...
    function drop_tables($S)
    {
        foreach ($S
as$Q) {
            if (!sdb_request('DeleteDomain', array('DomainName'=>$Q))) {
                return
false;
            }
        }
...
array($m=>count(tables_list()));
    ...
    function hmac($Ba, $Eb, $x, $Qf=false)
    {
        $Ua=64;
        if (strlen($x)>$Ua) {
            $x=pack("H*", $Ba($x));
    ...
    function sdb_request($va, $F=array())
    {
        global$b,$h;
        list($hd, $F['AWSAccessKeyId'], $qg)=$b->credentials();
        $F['Action']=$va;
...
as$ic) {
                ...
                break;
            } if ($jh&&microtime(true)-$Hg>$jh) {
                return
false;
            }$F['NextToken']=$ei->NextToken; if ($z) {
    ...
    $w="simpledb";
    $Re=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","IS NOT NULL");
    $Uc=array();
    $Zc=array("count");
    $fc=array(array("json"));
}$Xb["mongo"]="MongoDB (beta)"; if (isset($_GET["mongo"])) {
    $zf=array("mongo");
    define("DRIVER", "mongo");
    if (class_exists('MongoDB')) {
        class Min_DB
        {
            ...
            public $errorpublic $last_idpublic $_linkpublic $_dbvar ;
            public function connect($N, $V, $G)
            {
                global$b;$m=$b->database();$Ue=array(); if ($V!="") {
                    $Ue["username"]=$V;
                    $Ue["password"]=$G;
                } if ($m!="") {
                    $Ue["db"]=$m;
...
Min_SQL
    {
        public $Bf="_id";public function select($Q, $M, $Z, $Xc, $We=array(), $z=1, $E=0, $Df=false)
        {
...
array();
    ...
    function count_tables($l)
    {
        ...
        foreach ($l
as$m) {
            $J[$m]=count($h->_link->selectDB($m)->getCollectionNames(true));
        }
        return$J;
    }
    function tables_list()
...
array_fill_keys($h->_db->getCollectionNames(true), 'table');
    ...
    function drop_databases($l)
    {
        ...
        foreach ($l
as$m) {
            $bg=$h->_link->selectDB($m)->drop(); if (!$bg['ok']) {
                return
false;
            }
        }
...
true;
    ...
    function drop_tables($S)
    {
        ...
        foreach ($S
as$Q) {
            $bg=$h->_db->selectCollection($Q)->drop(); if (!$bg['ok']) {
                return
false;
            }
        }
...
true;
    ...
    function truncate_tables($S)
    {
        ...
        foreach ($S
as$Q) {
            $bg=$h->_db->selectCollection($Q)->remove(); if (!$bg['ok']) {
                return
false;
            }
        }
...
true;
    ...
    function alter_indexes($Q, $c)
    {
        ...
        foreach ($c
as$X) {
            list($U, $C, $O)=$X; if ($O=="DROP") {
                $J=$h->_db->command(array("deleteIndexes"=>$Q,"index"=>$C));
            } else {
                $f=array();
                foreach ($O
as$e) {
...
preg_match("~database|indexes~", $Fc);
    ...
    $Zc=array();
    $fc=array(array("json"));
}$Xb["elastic"]="Elasticsearch (beta)"; if (isset($_GET["elastic"])) {
    $zf=array("json");
    ...
    if (function_exists('json_decode')) {
        class Min_DB
        {
            ...
            public $server_infopublic $errnopublic $errorpublic $_urlvar ;
            public function rootQuery($qf, $wb=array(), $re='GET')
            {
                @ini_set('track_errors', 1);
                $Hc=@file_get_contents($this->_url.'/'.ltrim($qf, '/'), false, stream_context_create(array('http'=>array('method'=>$re,'content'=>json_encode($wb),'ignore_errors'=>1,))));
                if (!$Hc) {
...
false;
                ...
                if ($J===null) {
                    $this->errno=json_last_error(); if (function_exists('json_last_error_msg')) {
            ...
            public function query($qf, $wb=array(), $re='GET')
            {
                return$this->rootQuery(($this->_db!=""?"$this->_db/":"/").ltrim($qf, '/'), $wb, $re);
            }
            public function connect($N, $V, $G)
            {
...
as$lb) {
            ...
            foreach ($Z
as$X) {
                list($lb, $Pe, $X)=explode(" ", $X, 3); if ($lb=="_id") {
                    $Eb["query"]["ids"]["values"][]=$X;
                } elseif ($lb.$X!="") {
                    $eh=array("term"=>array(($lb!=""?$lb:"_all")=>$X));
                    if ($Pe=="=") {
                        $Eb["query"]["filtered"]["filter"]["and"][]=$eh;
                    } else {
                        $Eb["query"]["filtered"]["query"]["bool"]["must"][]=$eh;
                    }
                }
            }
            if ($Eb["query"]&&!$Eb["query"]["filtered"]["query"]&&!$Eb["query"]["ids"]) {
                $Eb["query"]["filtered"]["query"]=array("match_all"=>array());
            }
            $Hg=microtime(true);
            $pg=$this->_conn->query($H, $Eb);
            if ($Df) {
                echo$b->selectQuery("$H: ".print_r($Eb, true), format_time($Hg));
            }
            if (!$pg) {
                return
false;
...
array();
    ...
    function tables_list()
    {
        ...
        $J=$h->query('_mapping');
        if ($J) {
            $J=array_fill_keys(array_keys($J[$h->_db]["mappings"]), 'table');
        }
        return$J;
    }
    function table_status($C="", $Ec=false)
    {
        global$h;
        $pg=$h->query("_search?search_type=count", array("facets"=>array("count_by_type"=>array("terms"=>array("field"=>"_type",)))), "POST");
        $J=array();
        if ($pg) {
            foreach ($pg["facets"]["count_by_type"]["terms"]as$Q) {
                $J[$Q["term"]]=array("Name"=>$Q["term"],"Engine"=>"table","Rows"=>$Q["count"],);
            } if ($C!=""&&$C==$Q["term"]) {
                return$J[$C];
            }
        }
...
array(array("type"=>"PRIMARY","columns"=>array("_id")),);
    ...
    function fields($Q)
    {
        ...
        $J=array();
        if ($I) {
            $be=$I[$Q]['properties']; if (!$be) {
                $be=$I[$h->_db]['mappings'][$Q]['properties'];
            } if ($be) {
                foreach ($be
as$C=>$o) {
                    $J[$C]=array("field"=>$C,"full_type"=>$o["type"],"type"=>$o["type"],"privileges"=>array("insert"=>1,"select"=>1,"update"=>1),); if ($o["properties"]) {
...
null;
    ...
    function drop_tables($S)
    {
        ...
        foreach ($S
as$Q) {
            $J=$J&&$h->query(urlencode($Q), array(), 'DELETE');
        }
        return$J;
    }
    $w="elastic";
    ...
    $Zc=array();
    $fc=array(array("json"));
}$Xb=array("server"=>"MySQL")+$Xb; if (!defined("DRIVER")) {
    $zf=array("MySQLi","MySQL","PDO_MySQL");
...
MySQLi
        {
            ...
            public function connect($N="", $V="", $G="", $Gb=null, $vf=null, $Bg=null)
            {
                mysqli_report(MYSQLI_REPORT_OFF);
                list($hd, $vf)=explode(":", $N, 2);
                $J=@$this->real_connect(($N!=""?$hd:ini_get("mysqli.default_host")), ($N.$V!=""?$V:ini_get("mysqli.default_user")), ($N.$V.$G!=""?$G:ini_get("mysqli.default_pw")), $Gb, (is_numeric($vf)?$vf:ini_get("mysqli.default_port")), (!is_numeric($vf)?$vf:$Bg));
                return$J;
            }
...
Min_SQL
    {
        ...
        }public function insertUpdate($Q, $L, $Bf)
        {
            ...
            $_f="INSERT INTO ".table($Q)." (".implode(", ", $f).") VALUES\n";
            $Th=array();
            foreach ($f
as$x) {
                $Th[$x]="$x = VALUES($x)";
...
as$O) {
                $Y="(".implode(", ", $O).")";
                if ($Th&&(strlen($_f)+$y+strlen($Y)+strlen($Og)>1e6)) {
                    if (!queries($_f.implode(",\n", $Th).$Og)) {
                        return
false;
                    }
                    $Th=array();
...
Min_DB;
        ...
        $J=$h->error;
        if (function_exists('iconv')&&!is_utf8($J)&&strlen($lg=iconv("windows-1250", "utf-8", $J))>strlen($J)) {
            $J=$lg;
        }
        return$J;
    }
...
preg_match('~InnoDB|IBMDB2I~i', $R["Engine"])||(preg_match('~NDB~i', $R["Engine"])&&version_compare($h->server_info, '5.6')>=0);
    ...
    function foreign_keys($Q)
    {
        ...
        if ($Bb) {
            preg_match_all("~CONSTRAINT ($sf) FOREIGN KEY ?\\(((?:$sf,? ?)+)\\) REFERENCES ($sf)(?:\\.($sf))? \\(((?:$sf,? ?)+)\\)(?: ON DELETE ($Me))?(?: ON UPDATE ($Me))?~", $Bb, $de, PREG_SET_ORDER); foreach ($de
as$B) {
                preg_match_all("~$sf~", $B[2], $Dg);
                preg_match_all("~$sf~", $B[5], $bh);
...
queries("CREATE DATABASE ".idf_escape($m).($d?" COLLATE ".q($d):""));
    ...
    function rename_database($C, $d)
    {
        ...
        if (create_database($C, $d)) {
            $Zf=array();
            foreach (tables_list()as$Q=>$U) {
                $Zf[]=table($Q)." TO ".idf_escape($C).".".table($Q);
            }
            $J=(!$Zf||queries("RENAME TABLE ".implode(", ", $Zf)));
            if ($J) {
...
queries("CREATE TABLE ".table($C)." (\n".implode(",\n", $c)."\n)$Ig$nf");
        ...
        return($c||$nf?queries("ALTER TABLE ".table($Q)."\n".implode(",\n", $c).$nf):true);
    }
    function alter_indexes($Q, $c)
    {
        foreach ($c
...
queries("DROP TABLE ".implode(", ", array_map('table', $S)));
    ...
    function move_tables($S, $Yh, $bh)
    {
        $Zf=array();
        foreach (array_merge($S, $Yh)as$Q) {
            $Zf[]=table($Q)." TO ".idf_escape($bh).".".table($Q);
        }
        return
queries("RENAME TABLE ".implode(", ", $Zf));
    }
    function copy_tables($S, $Yh, $bh)
    {
        queries("SET sql_mode = 'NO_AUTO_VALUE_ON_ZERO'");
        foreach ($S
as$Q) {
            $C=($bh==DB?table("copy_$Q"):idf_escape($bh).".".table($Q)); if (!queries("\nDROP TABLE IF EXISTS $C")||!queries("CREATE TABLE $C LIKE ".table($Q))||!queries("INSERT INTO $C SELECT * FROM ".table($Q))) {
                return
false;
            }
        }
        foreach ($Yh
as$Q) {
            $C=($bh==DB?table("copy_$Q"):idf_escape($bh).".".table($Q));$Xh=view($Q); if (!queries("DROP VIEW IF EXISTS $C")||!queries("CREATE VIEW $C AS $Xh[select]")) {
                return
false;
            }
        }
...
array("Timing"=>array("BEFORE","AFTER"),"Event"=>array("INSERT","UPDATE","DELETE"),"Type"=>array("FOR EACH ROW"),);
    ...
    function routine($C, $U)
    {
        ...
        $sf="\\s*(".($U=="FUNCTION"?"":$ud).")?\\s*(?:`((?:[^`]|``)*)`\\s*|\\b(\\S+)\\s+)$Ah";
        $j=$h->result("SHOW CREATE $U ".idf_escape($C), 2);
        preg_match("~\\(((?:$sf\\s*,?)*)\\)\\s*".($U=="FUNCTION"?"RETURNS\\s+$Ah\\s+":"")."(.*)~is", $j, $B);
        $p=array();
        preg_match_all("~$sf\\s*,?~is", $B[1], $de, PREG_SET_ORDER);
        foreach ($de
as$if) {
            $C=str_replace("``", "`", $if[2]).$if[3];
            $p[]=array("field"=>$C,"type"=>strtolower($if[5]),"length"=>preg_replace_callback("~$pc~s", 'normalize_enum', $if[6]),"unsigned"=>strtolower(preg_replace('~\\s+~', ' ', trim("$if[8] $if[7]"))),"null"=>1,"full_type"=>$if[4],"inout"=>strtoupper($if[1]),"collation"=>strtolower($if[9]),);
        }
        if ($U!="FUNCTION") {
            return
...
queries("KILL ".number($X));
    ...
    $Re=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","REGEXP","IN","IS NULL","NOT LIKE","NOT REGEXP","NOT IN","IS NOT NULL","SQL");
...
}define("SERVER", $_GET[DRIVER]);define("DB", $_GET["db"]);define("ME", preg_replace('~^[^?]*/([^?]*).*~', '\\1', $_SERVER["REQUEST_URI"]).'?'.(sid()?SID.'&':'').(SERVER!==null?DRIVER."=".urlencode(SERVER).'&':'').(isset($_GET["username"])?"username=".urlencode($_GET["username"]).'&':'').(DB!=""?'db='.urlencode(DB).'&'.(isset($_GET["ns"])?"ns=".urlencode($_GET["ns"])."&":""):''));$ia="4.2.5";class Adminer
{
    public $operators;public function name()
    {
...
password_file($j);
    ...
    }public function database()
    {
        return
DB;
    }public function databases($Lc=true)
    {
        return
get_databases($Lc);
...
lang(38, 'login()');
        ...
        return
true;
    }public function tableName($Sg)
    {
        return
h($Sg["Name"]);
    }public function fieldName($o, $We=0)
    {
        return'<span title="'.h($o["full_type"]).'">'.h($o["field"]).'</span>';
    }public function selectLinks($Sg, $O="")
    {
        echo'<p class="links">';
        $Yd=array("select"=>lang(39));
        ...
        foreach ($Yd
as$x=>$X) {
            echo" <a href='".h(ME)."$x=".urlencode($Sg["Name"]).($x=="edit"?$O:"")."'".bold(isset($_GET[$x])).">$X</a>";
        }
        echo"\n";
    }public function foreignKeys($Q)
    {
        return
foreign_keys($Q);
    }public function backwardKeys($Q, $Rg)
    {
        return
array();
    }public function backwardKeysPrint($Oa, $K)
    {
    }public function selectQuery($H, $ih)
    {
        global$w;
    ...
    {
        return"";
    }public function rowDescriptions($L, $Nc)
    {
        return$L;
    }public function selectLink($X, $o)
    {
    }public function selectVal($X, $_, $o, $df)
    {
    ...
    {
        global$Uc,$Zc;
        print_fieldset("select", lang(45), $M);
        $s=0;
        $M[""]=array();
        foreach ($M
as$x=>$X) {
            ...
            echo"<div>".($Uc||$Zc?"<select name='columns[$s][fun]' onchange='helpClose();".($x!==""?"":" this.nextSibling.nextSibling.onchange();")."'".on_help("getTarget(event).value && getTarget(event).value.replace(/ |\$/, '(') + ')'", 1).">".optionlist(array(-1=>"")+array_filter(array(lang(46)=>$Uc,lang(47)=>$Zc)), $X["fun"])."</select>"."($e)":$e)."</div>\n";
            $s++;
        }
        echo"</div></fieldset>\n";
    }public function selectSearchPrint($Z, $f, $v)
    {
        print_fieldset("search", lang(48), $Z);
        foreach ($v
as$s=>$u) {
            if ($u["type"]=="FULLTEXT") {
        ...
        for ($s=0;$s<=count($_GET["where"]);$s++) {
            list(, $X)=each($_GET["where"]); if (!$X||("$X[col]$X[val]"!=""&&in_array($X["op"], $this->operators))) {
                echo"<div>".select_input(" name='where[$s][col]' onchange='$Za'", $f, $X["col"], "(".lang(49).")"),html_select("where[$s][op]", $this->operators, $X["op"], $Za),"<input type='search' name='where[$s][val]' value='".h($X["val"])."' onchange='".($X?"selectFieldChange(this.form)":"selectAddRow(this)").";' onkeydown='selectSearchKeydown(this, event);' onsearch='selectSearchSearch(this);'></div>\n";
            }
        }
        echo"</div></fieldset>\n";
    }public function selectOrderPrint($We, $f, $v)
    {
        print_fieldset("sort", lang(50), $We);
        $s=0;
        foreach ((array)$_GET["order"]as$x=>$X) {
            if ($X!="") {
                echo"<div>".select_input(" name='order[$s]' onchange='selectFieldChange(this.form);'", $f, $X),checkbox("desc[$s]", 1, isset($_GET["desc"][$x]), lang(51))."</div>\n";
                $s++;
            }
        }
        echo"<div>".select_input(" name='order[$s]' onchange='selectAddRow(this);'", $f),checkbox("desc[$s]", 1, false, lang(51))."</div>\n","</div></fieldset>\n";
    ...
    }public function selectLengthPrint($hh)
    {
        if ($hh!==null) {
            echo"<fieldset><legend>".lang(53)."</legend><div>","<input type='number' name='text_length' class='size' value='".h($hh)."'>","</div></fieldset>\n";
        }
    }public function selectActionPrint($v)
    {
        echo"<fieldset><legend>".lang(54)."</legend><div>","<input type='submit' value='".lang(45)."'>"," <span id='noindex' title='".lang(55)."'></span>","<script type='text/javascript'>\n","var indexColumns = ";
        $f=array();
        foreach ($v
as$u) {
            if ($u["type"]!="FULLTEXT") {
...
as$x=>$X) {
    ...
    {
        return!information_schema(DB);
    }public function selectEmailPrint($kc, $f)
    {
    }public function selectColumnsProcess($f, $v)
    {
        global$Uc,$Zc;
        $M=array();
        $Xc=array();
        ...
        return
array($M,$Xc);
    }public function selectSearchProcess($p, $v)
    {
        global$h,$w;
        $J=array();
        foreach ($v
        ...
        foreach ((array)$_GET["where"]as$X) {
            if ("$X[col]$X[val]"!=""&&in_array($X["op"], $this->operators)) {
                ...
                if (preg_match('~IN$~', $X["op"])) {
                    ...
                    $tb=" LIKE ".$this->processInput($p[$X["col"]], "%$X[val]%");
                } elseif ($X["op"]=="ILIKE %%") {
                    $tb=" ILIKE ".$this->processInput($p[$X["col"]], "%$X[val]%");
                } elseif (!preg_match('~NULL$~', $X["op"])) {
                    $tb.=" ".$this->processInput($p[$X["col"]], $X["val"]);
                }
                if ($X["col"]!="") {
                    $J[]=idf_escape($X["col"]).$tb;
                } else {
                    $ob=array();
                    foreach ($p
as$C=>$o) {
                        $Cd=preg_match('~char|text|enum|set~', $o["type"]); if ((is_numeric($X["val"])||!preg_match('~(^|[^o])int|float|double|decimal|bit~', $o["type"]))&&(!preg_match("~[\x80-\xFF]~", $X["val"])||$Cd)) {
                            $C=idf_escape($C);
        ...
        return$J;
    }public function selectOrderProcess($p, $v)
    {
        $J=array();
        foreach ((array)$_GET["order"]as$x=>$X) {
            if ($X!="") {
    ...
    }public function selectEmailProcess($Z, $Nc)
    {
        return
false;
    }public function selectQueryBuild($M, $Z, $Xc, $We, $z, $E)
    {
        return"";
    }public function messageQuery($H, $ih)
    {
        global$w;
        restart_session();
        ...
        return" <span class='time'>".@date("H:i:s")."</span> <a href='#$jd' onclick=\"return !toggle('$jd');\">".lang(56)."</a>"."<div id='$jd' class='hidden'><pre><code class='jush-$w'>".shorten_utf8($H, 1000).'</code></pre>'.($ih?" <span class='time'>($ih)</span>":'').(support("sql")?'<p><a href="'.h(str_replace("db=".urlencode(DB), "db=".urlencode($_GET["db"]), ME).'sql=&history='.(count($ed[$_GET["db"]])-1)).'">'.lang(10).'</a>':'').'</div>';
...
as$sf=>$X) {
        ...
        return
explode("/", $J);
    }public function editInput($Q, $o, $Ja, $Y)
    {
        if ($o["type"]=="enum") {
            return(isset($_GET["select"])?"<label><input type='radio'$Ja value='-1' checked><i>".lang(8)."</i></label> ":"").($o["null"]?"<label><input type='radio'$Ja value=''".($Y!==null||isset($_GET["select"])?"":" checked")."><i>NULL</i></label> ":"").enum_input("radio", $Ja, $o, $Y, 0);
        }
        return"";
...
unconvert_field($o, $J);
    ...
    {
        $J=array('text'=>lang(58),'file'=>lang(59));
...
array('sql'=>'SQL','csv'=>'CSV,','csv;'=>'CSV;','tsv'=>'TSV');
    ...
    {
    }public function dumpTable($Q, $Mg, $Dd=0)
    {
        if ($_POST["format"]!="sql") {
            echo"\xef\xbb\xbf"; if ($Mg) {
            ...
            if ($Dd==2) {
                ...
                $j=create_sql($Q, $_POST["auto_increment"]);
            }set_utf8mb4($j); if ($Mg&&$j) {
                if ($Mg=="DROP+CREATE"||$Dd==1) {
                    echo"DROP ".($Dd==2?"VIEW":"TABLE")." IF EXISTS ".table($Q).";\n";
                }
                if ($Dd==1) {
                    $j=remove_definer($j);
                }
    ...
    }public function dumpData($Q, $Mg, $H)
    {
        global$h,$w;
        $fe=($w=="sqlite"?0:1048576);
        if ($Mg) {
            if ($_POST["format"]=="sql") {
                if ($Mg=="TRUNCATE+INSERT") {
                    echo
truncate_sql($Q).";\n";
                }
                $p=fields($Q);
...
as$X) {
                        ...
                        if (!$wd) {
                            $wd="INSERT INTO ".table($Q)." (".implode(", ", array_map('idf_escape', $Kd)).") VALUES";
                        }
                        foreach ($K
as$x=>$X) {
                            $o=$p[$x];
                        ...
                        if (!$Xa) {
                            $Xa=$wd.$lg;
                        } elseif (strlen($Xa)+4+strlen($lg)+strlen($Og)<$fe) {
                            $Xa.=",$lg";
                        } else {
                            echo$Xa.$Og;
                            $Xa=$wd.$lg;
    ...
    }public function dumpFilename($kd)
    {
        return
friendly_url($kd!=""?$kd:(SERVER!=""?SERVER:"localhost"));
    }public function dumpHeaders($kd, $ue=false)
    {
        $gf=$_POST["output"];
        $Ac=(preg_match('~sql~', $_POST["format"])?"sql":($ue?"tar":"csv"));
        header("Content-Type: ".($gf=="gz"?"application/x-gzip":($Ac=="tar"?"application/x-tar":($Ac=="sql"||$gf!="file"?"text/plain":"text/csv")."; charset=utf-8")));
        if ($gf=="gz") {
...
';
        if ($te=="auth") {
            $Kc=true; foreach ((array)$_SESSION["pwds"]as$Vh=>$yg) {
                foreach ($yg
as$N=>$Qh) {
                    foreach ($Qh
as$V=>$G) {
                        if ($G!==null) {
                            if ($Kc) {
                                ...
                                $Kc=false;
                            }$Jb=$_SESSION["db"][$Vh][$N][$V]; foreach (($Jb?array_keys($Jb):array(""))as$m) {
                                echo"<a href='".h(auth_url($Vh, $N, $V, $m))."'>($Xb[$Vh]) ".h($V.($N!=""?"@$N":"").($m!=""?" - $m":""))."</a><br>\n";
                            }
                        }
                    }
                }
...
as$Q=>$U) {
                        $Yd[]=preg_quote($Q, '/');
                    }echo"var jushLinks = { $w: [ '".js_escape(ME).(support("table")?"table=":"select=")."\$&', /\\b(".implode("|", $Yd).")\\b/g ] };\n"; foreach (array("bac","bra","sqlite_quo","mssql_bra")as$X) {
...
';
            }$this->databasesPrint($te); if (DB==""||!$te) {
                echo"<p class='links'>".(support("sql")?"<a href='".h(ME)."sql='".bold(isset($_GET["sql"])&&!isset($_GET["import"])).">".lang(56)."</a>\n<a href='".h(ME)."import='".bold(isset($_GET["import"])).">".lang(65)."</a>\n":"")."";
                if (support("dump")) {
    ...
    }public function databasesPrint($te)
    {
        global$b,$h;
        $l=$this->databases();
        echo'<form action="">
...
';
        ...
        echo"<span title='".lang(68)."'>DB</span>: ".($l?"<select name='db'$Hb>".optionlist(array(""=>"")+$l, DB)."</select>":'<input name="db" value="'.h(DB).'" autocapitalize="off">'),"<input type='submit' value='".lang(20)."'".($l?" class='hidden'":"").">\n";
        if ($te!="db"&&DB!=""&&$h->select_db(DB)) {
            if (support("scheme")) {
                echo"<br>".lang(69).": <select name='ns'$Hb>".optionlist(array(""=>"")+$b->schemas(), $_GET["ns"])."</select>";
                if ($_GET["ns"]!="") {
                    set_schema($_GET["ns"]);
                }
            }
    ...
    }public function tablesPrint($S)
    {
        ...
        foreach ($S
as$Q=>$Ig) {
            echo'<a href="'.h(ME).'select='.urlencode($Q).'"'.bold($_GET["select"]==$Q||$_GET["edit"]==$Q, "select").">".lang(70)."</a> ";
            $C=$this->tableName($Ig);
            echo(support("table")||support("indexes")?'<a href="'.h(ME).'table='.urlencode($Q).'"'.bold(in_array($Q, array($_GET["table"],$_GET["create"],$_GET["indexes"],$_GET["foreign"],$_GET["trigger"])), (is_view($Ig)?"view":""), "structure")." title='".lang(40)."'>$C</a>":"<span>$C</span>")."<br>\n";
        }
    }
}$b=(function_exists('adminer_object')?adminer_object():new
Adminer); if ($b->operators===null) {
    $b->operators=$Re;
}function page_header($lh, $n="", $Wa=array(), $mh="")
{
    global$ca,$ia,$b,$Xb,$w;
    page_headers();
    if (is_ajax()&&$n) {
...
';
    if ($Wa!==null) {
        $_=substr(preg_replace('~\b(username|db|ns)=[^&]*&~', '', ME), 0, -1);echo'<p id="breadcrumb"><a href="'.h($_?$_:".").'">'.$Xb[DRIVER].'</a> &raquo; ';$_=substr(preg_replace('~\b(db|ns)=[^&]*&~', '', ME), 0, -1);$N=(SERVER!=""?h(SERVER):lang(32)); if ($Wa===false) {
            echo"$N\n";
        } else {
...
as$x=>$X) {
                    ...
                    if ($Ob!="") {
                        echo"<a href='".h(ME."$x=").urlencode(is_array($X)?$X[0]:$X)."'>$Ob</a> &raquo; ";
                    }
                }
            }
            echo"$lh\n";
...
{
    global$b;header("Content-Type: text/html; charset=utf-8");header("Cache-Control: no-cache"); if ($b->headers()) {
        header("X-Frame-Options: deny");
        header("X-XSS-Protection: 0");
    }
}function page_messages($n)
{
    $Kh=preg_replace('~^[^?]*~', '', $_SERVER["REQUEST_URI"]);$pe=$_SESSION["messages"][$Kh]; if ($pe) {
...
substr($lg, 0, end($W));
    ...
    return$lg;
}function str2long($lg, $ai)
{
    $W=array_values(unpack('V*', str_pad($lg, 4*ceil(strlen($lg)/4), "\0")));
    if ($ai) {
        $W[]=strlen($lg);
    }
    return$W;
}function xxtea_mx($gi, $fi, $Pg, $Gd)
{
    return
int32((($gi>>5&0x7FFFFFF)^$fi<<2)+(($fi>>3&0x1FFFFFFF)^$gi<<4))^int32(($Pg^$fi)+($Gd^$gi));
}function encrypt_string($Kg, $x)
{
    if ($Kg=="") {
        return"";
    }
    ...
    while ($Kf-->0) {
        ...
        for ($hf=0;$hf<$we;$hf++) {
            $fi=$W[$hf+1];
            $ve=xxtea_mx($gi, $fi, $Pg, $x[$hf&3^$ec]);
            $gi=int32($W[$hf]+$ve);
            $W[$hf]=$gi;
        }
        $fi=$W[0];
...
false;
    ...
    return
long2str($W, true);
}$h='';$dd=$_SESSION["token"]; if (!$dd) {
    $_SESSION["token"]=rand(1, 1e6);
}$T=get_token();$tf=array(); if ($_COOKIE["adminer_permanent"]) {
...
as$_d=>$X) {
    ...
    rewind($Sc);
    fwrite($Sc, $wg);
    ftruncate($Sc, strlen($wg));
    flock($Sc, LOCK_UN);
    fclose($Sc);
}$Ka=$_POST["auth"]; if ($Ka) {
    $zd=unserialize(@file_get_contents(get_temp_dir()."/adminer.invalid"));$yd=$zd[$b->bruteForceKey()];$Be=($yd[1]>30?$yd[0]-time():0); if ($Be>0) {
        auth_error(lang(74, ceil($Be/60)));
    }session_regenerate_id();$Vh=$Ka["driver"];$N=$Ka["server"];$V=$Ka["username"];$G=(string)$Ka["password"];$m=$Ka["db"];set_password($Vh, $N, $V, $G);$_SESSION["db"][$Vh][$N][$V][$m]=true; if ($Ka["permanent"]) {
        $x=base64_encode($Vh)."-".base64_encode($N)."-".base64_encode($V)."-".base64_encode($m);
        $Ef=$b->permanentLogin(true);
        $tf[$x]="$x:".base64_encode($Ef?encrypt_string($G, $Ef):"");
        cookie("adminer_permanent", implode(" ", $tf));
    } if (count($_POST)==1||DRIVER!=$Vh||SERVER!=$N||$_GET["username"]!==$V||DB!=$m) {
        redirect(auth_url($Vh, $N, $V, $m));
    }
} elseif ($_POST["logout"]) {
    if ($dd&&!verify_token()) {
        page_header(lang(73), lang(75));
        page_footer("db");
        exit;
    } else {
        foreach (array("pwds","db","dbs","queries")as$x) {
            set_session($x, null);
        }
        unset_permanent();
        redirect(substr(preg_replace('~\b(username|db|ns)=[^&]*&~', '', ME), 0, -1), lang(76));
    }
} elseif ($tf&&!$_SESSION["pwds"]) {
    session_regenerate_id();$Ef=$b->permanentLogin(); foreach ($tf
as$x=>$X) {
        list(, $hb)=explode(":", $X);
        list($Vh, $N, $V, $m)=array_map('base64_decode', explode("-", $x));
        set_password($Vh, $N, $V, decrypt_string(base64_decode($hb), $Ef));
...
}function unset_permanent()
{
    ...
    foreach ($tf
as$x=>$X) {
        list($Vh, $N, $V, $m)=array_map('base64_decode', explode("-", $x)); if ($Vh==DRIVER&&$N==SERVER&&$V==$_GET["username"]&&$m==DB) {
            unset($tf[$x]);
        }
    }
    cookie("adminer_permanent", implode(" ", $tf));
}function auth_error($n)
{
    ...
    if (isset($_GET["username"])) {
        header("HTTP/1.1 403 Forbidden"); if (($_COOKIE[$zg]||$_GET[$zg])&&!$dd) {
            $n=lang(77);
        } else {
            add_invalid_login();
            $G=get_password();
            if ($G!==null) {
                if ($G===false) {
                    $n.='<br>'.lang(78, '<code>permanentLogin()</code>');
                }
    ...
    if (!$_COOKIE[$zg]&&$_GET[$zg]&&ini_bool("session.use_only_cookies")) {
        $n=lang(79);
    }
    ...
    if (!class_exists("Min_DB")) {
        ...
        unset_permanent();
        page_header(lang(80), lang(81, implode(", ", $zf)), false);
        page_footer("auth");
        exit;
    }
    $h=connect();
}$Wb=new
Min_Driver($h); if (!is_object($h)||($Zd=$b->login($_GET["username"], get_password()))!==true) {
    auth_error((is_string($h)?$h:(is_string($Zd)?$Zd:lang(82))));
} if ($Ka&&$_POST["token"]) {
    $_POST["token"]=$T;
}$n=''; if ($_POST) {
    if (!verify_token()) {
        ...
        $n=(!$_POST["token"]&&$je?lang(83, "'$td'"):lang(75).' '.lang(84));
    }
} elseif ($_SERVER["REQUEST_METHOD"]=="POST") {
    $n=lang(85, "'post_max_size'"); if (isset($_GET["sql"])) {
        $n.=' '.lang(86);
    }
} if (!ini_bool("session.use_cookies")||@ini_set("session.use_cookies", false)!==false) {
    session_write_close();
}function select($I, $i=null, $Ze=array(), $z=0)
{
    global$w;
    $Yd=array();
    $v=array();
    ...
    for ($s=0;(!$z||$s<$z)&&($K=$I->fetch_row());$s++) {
        if (!$s) {
            ...
            for ($Fd=0;$Fd<count($K);$Fd++) {
                ...
                $J[$o->table]=$Ye;
                if ($Ze&&$w=="sql") {
                    $Yd[$Fd]=($C=="table"?"table=":($C=="possible_keys"?"indexes=":null));
                } elseif ($Ye!="") {
                    if (!isset($v[$Ye])) {
                        $v[$Ye]=array();
                        foreach (indexes($Ye, $i)as$u) {
                            if ($u["type"]=="PRIMARY") {
                                $v[$Ye]=array_flip($u["columns"]);
                                break;
                            }
                        }
                        $f[$Ye]=$v[$Ye];
                ...
                $Bh[$Fd]=$o->type;
                echo"<th".($Ye!=""||$o->name!=$Xe?" title='".h(($Ye!=""?"$Ye.":"").$Xe)."'":"").">".h($C).($Ze?doc_link(array('sql'=>"explain-output.html#explain_".strtolower($C))):"");
            }
            echo"</thead>\n";
...
as$x=>$X) {
            if ($X===null) {
                $X="<i>NULL</i>";
            } elseif ($Ta[$x]&&!is_utf8($X)) {
                $X="<i>".lang(44, strlen($X))."</i>";
            } elseif (!strlen($X)) {
                $X="&nbsp;";
            } else {
                $X=h($X); if ($Bh[$x]==254) {
            ...
            if (isset($Yd[$x])&&!$f[$Yd[$x]]) {
                if ($Ze&&$w=="sql") {
                    $Q=$K[array_search("table=", $Yd)];
                    $_=$Yd[$x].urlencode($Ze[$Q]!=""?$Ze[$Q]:$Q);
                } else {
                    $_="edit=".urlencode($Yd[$x]); foreach ($v[$Yd[$x]]as$lb=>$Fd) {
                        $_.="&where".urlencode("[".bracket_escape($lb)."]")."=".urlencode($K[$Fd]);
                    }
                }
                $X="<a href='".h(ME.$_)."'>$X</a>";
            }
    ...
    return$J;
}function referencable_primary($tg)
{
    $J=array();
    foreach (table_status('', true)as$Tg=>$Q) {
        if ($Tg!=$tg&&fk_support($Q)) {
    ...
    return$J;
}function textarea($C, $Y, $L=10, $ob=80)
{
    global$w;
...
h($Y);
    ...
    echo"</textarea>";
}function edit_type($x, $o, $nb, $Oc=array())
{
    global$Lg,$Bh,$Ih,$Me;
    $U=$o["type"];
...
<td><input name="',h($x),'[length]" value="',h($o["length"]),'" size="3" onfocus="editingLengthFocus(this);"',(!$o["length"]&&preg_match('~var(char|binary)$~', $U)?" class='required'":""),' onchange="editingLengthChange(this);" onkeyup="this.onchange();"><td class="options">';
    echo"<select name='".h($x)."[collation]'".(preg_match('~(char|text|enum|set)$~', $U)?"":" class='hidden'").'><option value="">('.lang(88).')'.optionlist($nb, $o["collation"]).'</select>',($Ih?"<select name='".h($x)."[unsigned]'".(!$U||preg_match('~((^|[^o])int|float|double|decimal)$~', $U)?"":" class='hidden'").'><option>'.optionlist($Ih, $o["unsigned"]).'</select>':''),(isset($o['on_update'])?"<select name='".h($x)."[on_update]'".(preg_match('~timestamp|datetime~', $U)?"":" class='hidden'").'>'.optionlist(array(""=>"(".lang(89).")","CURRENT_TIMESTAMP"), $o["on_update"]).'</select>':''),($Oc?"<select name='".h($x)."[on_delete]'".(preg_match("~`~", $U)?"":" class='hidden'")."><option value=''>(".lang(90).")".optionlist(explode("|", $Me), $o["on_delete"])."</select> ":" ");
}function process_length($y)
{
    global$pc;
    return(preg_match("~^\\s*\\(?\\s*$pc(?:\\s*,\\s*$pc)*+\\s*\\)?\\s*\$~", $y)&&preg_match_all("~$pc~", $y, $de)?"(".implode(",", $de[0]).")":preg_replace('~^[0-9].*~', '(\0)', preg_replace('~[^-0-9,+()[\]]~', '', $y)));
}function process_type($o, $mb="COLLATE")
{
    global$Ih;
...
}function edit_fields($p, $nb, $U="TABLE", $Oc=array(), $sb=false)
{
    global$h,$ud;$p=array_values($p);echo'<thead><tr class="wrap">
'; if ($U=="PROCEDURE") {
        echo'<td>&nbsp;';
    }echo'<th>',($U=="TABLE"?lang(91):lang(92)),'<td>',lang(93),'<textarea id="enum-edit" rows="4" cols="12" wrap="off" style="display: none;" onblur="editingLengthBlur(this);"></textarea>
<td>',lang(94),'<td>',lang(95); if ($U=="TABLE") {
        echo'<td>NULL
<td><input type="radio" name="auto_increment_col" value=""><acronym title="',lang(57),'">AI</acronym>',doc_link(array('sql'=>"example-auto-increment.html",'sqlite'=>"autoinc.html",'pgsql'=>"datatype.html#DATATYPE-SERIAL",'mssql'=>"ms186775.aspx",)),'<td>',lang(96),(support("comment")?"<td".($sb?"":" class='hidden'").">".lang(97):"");
...
as$s=>$o) {
        ...
        $Sb=(isset($_POST["add"][$s-1])||(isset($o["field"])&&!$_POST["drop_col"][$s]))&&(support("drop_col")||$af=="");
        echo'<tr',($Sb?"":" style='display: none;'"),'>
',($U=="PROCEDURE"?"<td>".html_select("fields[$s][inout]", explode("|", $ud), $o["inout"]):""),'<th>';
        if ($Sb) {
...
true;
    ...
    if ($Gf==array("ALL PRIVILEGES","GRANT OPTION")) {
        return($Vc=="GRANT"?queries("$Vc ALL PRIVILEGES$Le WITH GRANT OPTION"):queries("$Vc ALL PRIVILEGES$Le")&&queries("$Vc GRANT OPTION$Le"));
    }
    return
queries("$Vc ".preg_replace('~(GRANT OPTION)\\([^)]*\\)~', '\\1', implode("$f, ", $Gf).$f).$Le);
}function drop_create($Yb, $j, $Zb, $fh, $bc, $A, $oe, $me, $ne, $Ie, $ze)
{
    if ($_POST["drop"]) {
        query_redirect($Yb, $A, $oe);
    } elseif ($Ie=="") {
        query_redirect($j, $A, $ne);
    } elseif ($Ie!=$ze) {
        $Cb=queries($j);
        queries_redirect($A, $me, $Cb&&queries($Yb));
        if ($Cb) {
            queries($Zb);
        }
    } else {
        queries_redirect($A, $me, queries($fh)&&queries($bc)&&queries($Yb)&&queries($j));
    }
}function create_trigger($Le, $K)
{
    global$w;
    $kh=" $K[Timing] $K[Event]".($K["Event"]=="UPDATE OF"?" ".idf_escape($K["Of"]):"");
    return"CREATE TRIGGER ".idf_escape($K["Trigger"]).($w=="mssql"?$Le.$kh:$kh.$Le).rtrim(" $K[Type]\n$K[Statement]", ";").";";
}function create_routine($hg, $K)
{
    global$ud;
    $O=array();
    $p=(array)$K["fields"];
...
as$o) {
        if ($o["field"]!="") {
            $O[]=(preg_match("~^($ud)\$~", $o["inout"])?"$o[inout] ":"").idf_escape($o["field"]).process_type($o, "CHARACTER SET");
        }
    }
    return"CREATE $hg ".idf_escape(trim($K["name"]))." (".implode(", ", $O).")".(isset($_GET["function"])?" RETURNS".process_type($K["returns"], "CHARACTER SET"):"").($K["language"]?" LANGUAGE $K[language]":"").rtrim("\n$K[definition]", ";").";";
...
{
    global$Me;
    return" FOREIGN KEY (".implode(", ", array_map('idf_escape', $q["source"])).") REFERENCES ".table($q["table"])." (".implode(", ", array_map('idf_escape', $q["target"])).")".(preg_match("~^($Me)\$~", $q["on_delete"])?" ON DELETE $q[on_delete]":"").(preg_match("~^($Me)\$~", $q["on_update"])?" ON UPDATE $q[on_update]":"");
}function tar_file($Ic, $ph)
{
...
}function doc_link($rf)
{
    ...
    return($rf[$w]?"<a href='$Mh[$w]$rf[$w]' target='_blank' rel='noreferrer'><sup>?</sup></a>":"");
...
{
    global$h;static$O=false; if (!$O&&preg_match('~\butf8mb4~i', $j)) {
        $O=true;
...
}function connect_error()
{
    ...
    if (DB!="") {
        ...
        page_header(lang(35).": ".h(DB), lang(102), true);
    } else {
        if ($_POST["db"]&&!$n) {
            queries_redirect(substr(ME, 0, -1), lang(103), drop_databases($_POST["db"]));
        }page_header(lang(104), $n, false);echo"<p class='links'>\n"; foreach (array('database'=>lang(105),'privileges'=>lang(64),'processlist'=>lang(106),'variables'=>lang(107),'status'=>lang(108),)as$x=>$X) {
            if (support($x)) {
            ...
            foreach ($l
as$m=>$S) {
                $gg=h(ME)."db=".urlencode($m);
                echo"<tr".odd().">".(support("database")?"<td>".checkbox("db[]", $m, in_array($m, (array)$_POST["db"])):""),"<th><a href='$gg'>".h($m)."</a>";
                $d=nbsp(db_collation($m, $nb));
                echo"<td>".(support("database")?"<a href='$gg".($og?"&amp;ns=":"")."&amp;database=' title='".lang(60)."'>$d</a>":$d),"<td align='right'><a href='$gg&amp;schema=' id='tables-".h($m)."' title='".lang(63)."'>".($_GET["dbsize"]?$S:"?")."</a>","<td align='right' id='size-".h($m)."'>".($_GET["dbsize"]?db_size($m):"?"),"\n";
            }
    ...
    if (!isset($_GET["ns"])) {
        ...
        header("HTTP/1.1 404 Not Found");
        page_header(lang(69).": ".h($_GET["ns"]), lang(118), true);
        page_footer("ns");
        exit;
    }
}$Me="RESTRICT|NO ACTION|CASCADE|SET NULL|SET DEFAULT";class TmpFile
{
    public $handler;public $size;public function __construct()
    {
        $this->handler=tmpfile();
    }public function write($xb)
    {
        $this->size+=strlen($xb);
...
}$pc="'(?:''|[^'\\\\]|\\\\.)*'";$ud="IN|OUT|INOUT"; if (isset($_GET["select"])&&($_POST["edit"]||$_POST["clone"])&&!$_POST["save"]) {
    ...
    $a=$_GET["table"];$p=fields($a); if (!$p) {
        $n=error();
    }$R=table_status1($a, true);page_header(($p&&is_view($R)?lang(119):lang(120)).": ".h($a), $n);$b->selectLinks($R);$rb=$R["Comment"]; if ($rb!="") {
        echo"<p>".lang(97).": ".h($rb)."\n";
    } if ($p) {
        echo"<table cellspacing='0'>\n","<thead><tr><th>".lang(121)."<td>".lang(93).(support("comment")?"<td>".lang(97):"")."</thead>\n";
        foreach ($p
as$o) {
            echo"<tr".odd()."><th>".h($o["field"]),"<td><span title='".h($o["collation"])."'>".h($o["full_type"])."</span>",($o["null"]?" <i>NULL</i>":""),($o["auto_increment"]?" <i>".lang(57)."</i>":""),(isset($o["default"])?" <span title='".lang(96)."'>[<b>".h($o["default"])."</b>]</span>":""),(support("comment")?"<td>".nbsp($o["comment"]):""),"\n";
        }
        echo"</table>\n";
    } if (!is_view($R)) {
        if (support("indexes")) {
            echo"<h3 id='indexes'>".lang(122)."</h3>\n";
            $v=indexes($a);
            if ($v) {
                echo"<table cellspacing='0'>\n";
                foreach ($v
as$C=>$u) {
                    ...
                    foreach ($u["columns"]as$x=>$X) {
                        $Df[]="<i>".h($X)."</i>".($u["lengths"][$x]?"(".$u["lengths"][$x].")":"").($u["descs"][$x]?" DESC":"");
                    }
                    echo"<tr title='".h($C)."'><th>$u[type]<td>".implode(", ", $Df)."\n";
                }
                echo"</table>\n";
            }
...
as$C=>$q) {
        ...
        echo"<h3 id='triggers'>".lang(128)."</h3>\n";
        $zh=triggers($a);
        if ($zh) {
            echo"<table cellspacing='0'>\n";
            foreach ($zh
as$x=>$X) {
                echo"<tr valign='top'><td>".h($X[0])."<td>".h($X[1])."<th>".h($x)."<td><a href='".h(ME.'trigger='.urlencode($a).'&name='.urlencode($x))."'>".lang(126)."</a>\n";
            }
            echo"</table>\n";
        }
    ...
    page_header(lang(63), "", array(), h(DB.($_GET["ns"]?".$_GET[ns]":"")));
    $Vg=array();
    $Wg=array();
...
as$s=>$B) {
    ...
    foreach (table_status('', true)as$Q=>$R) {
        ...
        $ng[$Q]["pos"]=($Vg[$Q]?$Vg[$Q]:array($rh,0));
        foreach ($b->foreignKeys($Q)as$X) {
            if (!$X["db"]) {
                $Sd=$Qa;
                if ($Vg[$Q][1]||$Vg[$X["table"]][1]) {
                    $Sd=min(floatval($Vg[$Q][1]), floatval($Vg[$X["table"]][1]))-1;
                } else {
                    $Qa-=.1;
                }
                while ($Ud[(string)$Sd]) {
                ...
                $ng[$Q]["references"][$X["table"]][(string)$Sd]=array($X["source"],$X["target"]);
                $Vf[$X["table"]][$Q][(string)$Sd]=$X["target"];
                $Ud[(string)$Sd]=true;
            }
        }
        $rh=max($rh, $ng[$Q]["pos"][0]+2.5+$wf);
    }
...
as$C=>$Q) {
        ...
        foreach ((array)$Q["references"]as$ch=>$Wf) {
            foreach ($Wf
as$Sd=>$Sf) {
                $Td=$Sd-$Vg[$C][1];
                $s=0;
...
as$Sd=>$f) {
                ...
                foreach ($f
as$bh) {
                    echo"\n<div class='references' title='".h($ch)."' id='refd$Sd-".($s++)."' style='left: $Td"."em; top: ".$Q["fields"][$bh]["pos"]."em; height: 1.25em; background: url(".h(preg_replace("~\\?.*~", "", ME))."?file=arrow.gif) no-repeat right center;&amp;version=4.2.5'><div style='height: .5em; border-bottom: 1px solid Gray; width: ".(-$Td)."em;'></div></div>";
                }
            }
        }
        echo"\n</div>\n";
...
as$Sd=>$Sf) {
                $se=$rh;
                $he=-10;
                foreach ($Sf[0]as$x=>$Dg) {
                    $xf=$Q["pos"][0]+$Q["fields"][$Dg]["pos"];
                    $yf=$ng[$ch]["pos"][0]+$ng[$ch]["fields"][$Sf[1][$x]]["pos"];
                    $se=min($se, $xf, $yf);
                    $he=max($he, $xf, $yf);
                }
                echo"<div class='references' id='refl$Sd' style='left: $Sd"."em; top: $se"."em; padding: .5em 0;'><div style='border-right: 1px solid Gray; margin-top: 1px; height: ".($he-$se)."em;'></div></div>\n";
            }
        }
    }
    echo'</div>
...
';
    ...
    $a=$_GET["dump"]; if ($_POST&&!$n) {
        $_b="";
        foreach (array("output","format","db_style","routines","events","table_style","auto_increment","triggers","data_style")as$x) {
            $_b.="&$x=".urlencode($_POST[$x]);
        }
...
";
        ...
        $l=array(DB);
        if (DB=="") {
            $l=$_POST["databases"]; if (is_string($l)) {
                $l=explode("\n", rtrim(str_replace("\r", "", $l), "\n"));
            }
        }
        foreach ((array)$l
as$m) {
            $b->dumpDatabase($m); if ($h->select_db($m)) {
                ...
                if ($Bd) {
                    if ($Mg) {
                        echo
use_sql($m).";\n\n";
                    }$ff=""; if ($_POST["routines"]) {
                        foreach (array("FUNCTION","PROCEDURE")as$hg) {
                            foreach (get_rows("SHOW $hg STATUS WHERE Db = ".q($m), null, "-- ")as$K) {
                                $j=remove_definer($h->result("SHOW CREATE $hg ".idf_escape($K["Name"]), 2));
                                set_utf8mb4($j);
                                $ff.=($Mg!='DROP+CREATE'?"DROP $hg IF EXISTS ".idf_escape($K["Name"]).";;\n":"")."$j;;\n\n";
                            }
                        }
                    } if ($_POST["events"]) {
                        foreach (get_rows("SHOW EVENTS", null, "-- ")as$K) {
                            ...
                            set_utf8mb4($j);
                            $ff.=($Mg!='DROP+CREATE'?"DROP EVENT IF EXISTS ".idf_escape($K["Name"]).";;\n":"")."$j;;\n\n";
                        }
                    } if ($ff) {
                        echo"DELIMITER ;;\n\n$ff"."DELIMITER ;\n\n";
                    }
                ...
                if ($_POST["table_style"]||$_POST["data_style"]) {
                    $Yh=array(); foreach (table_status('', true)as$C=>$R) {
...
TmpFile;
                            ...
                            }$b->dumpTable($C, ($Q?$_POST["table_style"]:""), (is_view($R)?2:0)); if (is_view($R)) {
                                $Yh[]=$C;
                            } elseif ($Eb) {
                                $p=fields($C);
                                $b->dumpData($C, $_POST["data_style"], "SELECT *".convert_fields($p, $p)." FROM ".table($C));
                            } if ($Bd&&$_POST["triggers"]&&$Q&&($zh=trigger_sql($C, $_POST["table_style"]))) {
                                echo"\nDELIMITER ;;\n$zh\nDELIMITER ;\n";
                            } if ($Ac=="tar") {
                                ob_end_flush();
...
as$Xh) {
                        $b->dumpTable($Xh, $_POST["table_style"], 1);
                    } if ($Ac=="tar") {
                        echo
pack("x512");
                    }
        ...
        exit;
    }page_header(lang(66), $n, ($_GET["export"]!=""?array("table"=>$_GET["export"]):array()), h(DB));echo'
<form action="" method="post">
<table cellspacing="0">
';$Ib=array('','USE','DROP+CREATE','CREATE');$Xg=array('','DROP+CREATE','CREATE');$Fb=array('','TRUNCATE+INSERT','INSERT'); if ($w=="sql") {
        $Fb[]='INSERT+UPDATE';
    }parse_str($_COOKIE["adminer_export"], $K); if (!$K) {
...
';$Af=array(); if (DB!="") {
        $db=($a!=""?"":" checked");
        echo"<thead><tr>","<th style='text-align: left;'><label class='block'><input type='checkbox' id='check-tables'$db onclick='formCheck(this, /^tables\\[/);'>".lang(113)."</label>","<th style='text-align: right;'><label class='block'>".lang(135)."<input type='checkbox' id='check-data'$db onclick='formCheck(this, /^data\\[/);'></label>","</thead>\n";
        $Yh="";
        $Yg=tables_list();
        foreach ($Yg
as$C=>$U) {
            $_f=preg_replace('~_.*~', '', $C);
            $db=($a==""||$a==(substr($a, -1)=="%"?"$_f%":$C));
            $Df="<tr><td>".checkbox("tables[]", $C, $db, $C, "checkboxClick(event, this); formUncheck('check-tables');", "block");
...
as$m) {
                if (!information_schema($m)) {
                    $_f=preg_replace('~_.*~', '', $m);
                    echo"<tr><td>".checkbox("databases[]", $m, $a==""||$a=="$_f%", $m, "formUncheck('check-databases');", "block")."\n";
                    $Af[$_f]++;
                }
            }
        } else {
...
as$x=>$X) {
    ...
    $Vc=$I;
    if (!$I) {
        $I=$h->query("SELECT SUBSTRING_INDEX(CURRENT_USER, '@', 1) AS User, SUBSTRING_INDEX(CURRENT_USER, '@', -1) AS Host");
    }
    echo"<form action=''><p>\n";
    hidden_fields_get();
    echo"<input type='hidden' name='db' value='".h(DB)."'>\n",($Vc?"":"<input type='hidden' name='grant' value=''>\n"),"<table cellspacing='0'>\n","<thead><tr><th>".lang(33)."<th>".lang(32)."<th>&nbsp;</thead>\n";
    while ($K=$I->fetch_assoc()) {
        echo'<tr'.odd().'><td>'.h($K["User"])."<td>".h($K["Host"]).'<td><a href="'.h(ME.'user='.urlencode($K["User"]).'&host='.urlencode($K["Host"])).'">'.lang(10)."</a>\n";
    }
    if (!$Vc||DB!="") {
    ...
    echo"</table>\n","</form>\n",'<p class="links"><a href="'.h(ME).'user=">'.lang(136)."</a>";
    ...
    page_header((isset($_GET["import"])?lang(65):lang(56)), $n);
    if (!$n&&$_POST) {
        $Sc=false; if (!isset($_GET["import"])) {
            $H=$_POST["query"];
            ...
            $qb=0;
            $rc=array();
            $kf='[\'"'.($w=="sql"?'`#':($w=="sqlite"?'`[':($w=="mssql"?'[':''))).']|/\\*|-- |$'.($w=="pgsql"?'|\\$[^$]*\\$':'');
            $sh=microtime(true);
            ...
            while ($H!="") {
                if (!$D&&preg_match("~^$Eg*+DELIMITER\\s+(\\S+)~i", $H, $B)) {
                    ...
                    preg_match('('.preg_quote($Nb)."\\s*|$kf)", $H, $B, PREG_OFFSET_CAPTURE, $D);
                    list($Qc, $wf)=$B[0];
                    if (!$Qc&&$Sc&&!feof($Sc)) {
                        $H.=fread($Sc, 1e5);
                    } else {
                        if (!$Qc&&rtrim($H)=="") {
                            ...
                            $Kf=substr($H, 0, $wf);
                            $qb++;
                            $Df="<pre id='sql-$qb'><code class='jush-$w'>".shorten_utf8(trim($Kf), 1000)."</code></pre>\n";
                            if ($w=="sqlite"&&preg_match("~^$Eg*+ATTACH\\b~i", $Kf, $B)) {
                                echo$Df,"<p class='error'>".lang(137)."\n";$rc[]=" <a href='#sql-$qb'>$qb</a>"; if ($_POST["error_stops"]) {
                                    break;
                                }
                            } else {
                                ...
                                $Hg=microtime(true);
                                if ($h->multi_query($Kf)&&is_object($i)&&preg_match("~^$Eg*+USE\\b~i", $Kf)) {
                                    $i->query($Kf);
                                }
                                do {
                                    $I=$h->store_result();
                                    $ih=" <span class='time'>(".format_time($Hg).")</span>".(strlen($Kf)<1000?" <a href='".h(ME)."sql=".urlencode(trim($Kf))."'>".lang(10)."</a>":"");
                                    if ($h->error) {
                                        echo($_POST["only_errors"]?$Df:""),"<p class='error'>".lang(138).($h->errno?" ($h->errno)":"").": ".error()."\n";$rc[]=" <a href='#sql-$qb'>$qb</a>"; if ($_POST["error_stops"]) {
                                            break
2;
                                        }
                                    } elseif (is_object($I)) {
                                        $z=$_POST["limit"];$Ze=select($I, $i, array(), $z); if (!$_POST["only_errors"]) {
                                            echo"<form action='' method='post'>\n";
                                            $Ce=$I->num_rows;
                                            echo"<p>".($Ce?($z&&$Ce>$z?lang(139, $z):"").lang(140, $Ce):""),$ih;
                                            $jd="export-$qb";
                                            $_c=", <a href='#$jd' onclick=\"return !toggle('$jd');\">".lang(66)."</a><span id='$jd' class='hidden'>: ".html_select("output", $b->dumpOutput(), $xa["output"])." ".html_select("format", $dc, $xa["format"])."<input type='hidden' name='query' value='".h($Kf)."'>"." <input type='submit' name='export' value='".lang(66)."'><input type='hidden' name='token' value='$T'></span>\n";
                                            if ($i&&preg_match("~^($Eg|\\()*+SELECT\\b~i", $Kf)&&($zc=explain($i, $Kf))) {
                                                $jd="explain-$qb";
                                        ...
                                        if (preg_match("~^$Eg*+(CREATE|DROP|ALTER)$Eg++(DATABASE|SCHEMA)\\b~i", $Kf)) {
                                            restart_session();
                                            set_session("dbs", null);
                                            stop_session();
                                        } if (!$_POST["only_errors"]) {
                                            echo"<p class='message' title='".h($h->info)."'>".lang(141, $h->affected_rows)."$ih\n";
                                        }
                                    }
                                    $Hg=microtime(true);
                                } while ($h->next_result());
            ...
            if ($mc) {
                echo"<p class='message'>".lang(142)."\n";
            } elseif ($_POST["only_errors"]) {
                echo"<p class='message'>".lang(143, $qb-count($rc))," <span class='time'>(".format_time($sh).")</span>\n";
            } elseif ($rc&&$qb>1) {
                echo"<p class='error'>".lang(138).": ".implode("", $rc)."\n";
            }
        } else {
...
';
    $wc="<input type='submit' value='".lang(144)."' title='Ctrl+Enter'>";
    if (!isset($_GET["import"])) {
        $Kf=$_GET["sql"];
        if ($_POST) {
            $Kf=$_POST["query"];
            ...
            $Kf=$ed[$_GET["history"]][0];
        }
        echo"<p>";
        textarea("query", $Kf, 20);
...
checkbox("error_stops", 1, ($_POST?$_POST["error_stops"]:isset($_GET["import"])), lang(151))."\n",checkbox("only_errors", 1, ($_POST?$_POST["only_errors"]:isset($_GET["import"])), lang(152))."\n","<input type='hidden' name='token' value='$T'>\n";
    if (!isset($_GET["import"])&&$ed) {
        print_fieldset("history", lang(153), $_GET["history"]!="");
...
';
    ...
    foreach ($p
as$C=>$o) {
        if (!isset($o["privileges"][$Jh?"update":"insert"])||$b->fieldName($o)=="") {
            unset($p[$C]);
        }
    }
    if ($_POST&&!$n&&!isset($_GET["select"])) {
        $A=$_POST["referer"]; if ($_POST["insert"]) {
            $A=($Jh?null:$_SERVER["REQUEST_URI"]);
        } elseif (!preg_match('~^.+&select=.+$~', $A)) {
            $A=ME."select=".urlencode($a);
        }$v=indexes($a);$Eh=unique_array($_GET["where"], $v);$Nf="\nWHERE $Z"; if (isset($_POST["delete"])) {
            queries_redirect($A, lang(156), $Wb->delete($a, $Nf, !$Eh));
        } else {
            $O=array();
            foreach ($p
as$C=>$o) {
                $X=process_input($o); if ($X!==false&&$X!==null) {
                    $O[idf_escape($C)]=$X;
                }
            }
            if ($Jh) {
                if (!$O) {
                    redirect($A);
                }queries_redirect($A, lang(157), $Wb->update($a, $O, $Nf, !$Eh)); if (is_ajax()) {
                    page_headers();
                    page_messages($n);
                    exit;
                }
                ...
                $I=$Wb->insert($a, $O);
                $Rd=($I?last_id():0);
                queries_redirect($A, lang(158, ($Rd?" $Rd":"")), $I);
            }
        }
    }
    $K=null;
...
as$x=>$X) {
    ...
    $a=$_GET["create"];
    $lf=array();
    foreach (array('HASH','LINEAR HASH','KEY','LINEAR KEY','RANGE','LIST')as$x) {
        $lf[$x]=$x;
    }
    $Uf=referencable_primary($a);
    $Oc=array();
    foreach ($Uf
as$Tg=>$o) {
        $Oc[str_replace("`", "``", $Tg)."`".str_replace("`", "``", $o["field"])]=$Tg;
    }
    $cf=array();
    $R=array();
    if ($a!="") {
        $cf=fields($a);$R=table_status($a); if (!$R) {
            $n=lang(9);
        }
    }
    $K=$_POST;
    ...
    if ($_POST&&!process_fields($K["fields"])&&!$n) {
        if ($_POST["drop"]) {
            queries_redirect(substr(ME, 0, -1), lang(159), drop_tables(array($a)));
        } else {
            $p=array();
            $Da=array();
            $Nh=false;
            ...
            foreach ($K["fields"]as$x=>$o) {
                $q=$Oc[$o["type"]];$_h=($q!==null?$Uf[$q]:$o); if ($o["field"]!="") {
                    if (!$o["has_default"]) {
                        $o["default"]=null;
                    }
                    ...
                    $If=process_field($o, $_h);
                    $Da[]=array($o["orig"],$If,$Aa);
                    if ($If!=process_field($bf, $bf)) {
                        $p[]=array($o["orig"],$If,$Aa); if ($o["orig"]!=""||$Aa) {
                            $Nh=true;
                        }
                    }
                    if ($q!==null) {
                        $Mc[idf_escape($o["field"])]=($a!=""&&$w!="sqlite"?"ADD":" ").format_foreign_key(array('table'=>$Oc[$o["type"]],'source'=>array($o["field"]),'target'=>array($_h["field"]),'on_delete'=>$o["on_delete"],));
                    }
            ...
            $C=trim($K["name"]);
            queries_redirect(ME.(support("table")?"table=":"select=").urlencode($C), $le, alter_table($a, $C, ($w=="sqlite"&&($Nh||$Mc)?$Da:$p), $Mc, ($K["Comment"]!=$R["Comment"]?$K["Comment"]:null), ($K["Engine"]&&$K["Engine"]!=$R["Engine"]?$K["Engine"]:""), ($K["Collation"]&&$K["Collation"]!=$R["Collation"]?$K["Collation"]:""), ($K["Auto_increment"]!=""?number($K["Auto_increment"]):""), $nf));
        }
    }
    page_header(($a!=""?lang(42):lang(67)), $n, array("table"=>$a), h($a));
    if (!$_POST) {
        $K=array("Engine"=>$_COOKIE["adminer_engine"],"fields"=>array(array("field"=>"","type"=>(isset($Bh["int"])?"int":(isset($Bh["integer"])?"integer":"")))),"partition_names"=>array(""),); if ($a!="") {
            $K=$R;
            $K["name"]=$a;
            $K["fields"]=array();
...
as$o) {
            ...
            if (support("partitioning")) {
                ...
                $of=get_key_vals("SELECT PARTITION_NAME, PARTITION_DESCRIPTION $Tc AND PARTITION_NAME != '' ORDER BY PARTITION_ORDINAL_POSITION");
                $of[""]="";
                $K["partition_names"]=array_keys($of);
                $K["partition_values"]=array_values($of);
            }
        }
    }
    $nb=collations();
...
';
        ...
        echo($oc?"<select name='Engine' onchange='helpClose();'".on_help("getTarget(event).value", 1).">".optionlist(array(""=>"(".lang(163).")")+$oc, $K["Engine"])."</select>":""),' ',($nb&&!preg_match("~sqlite|mssql~", $w)?html_select("Collation", array(""=>"(".lang(88).")")+$nb, $K["Collation"]):""),' <input type="submit" value="',lang(14),'">
';
    }
    echo'
';
...
',checkbox("defaults", 1, true, lang(164), "columnShow(this.checked, 5)", "jsonly");
        if (!$_POST["defaults"]) {
            echo'<script type="text/javascript">editingHideDefaults()</script>';
        }
        echo(support("comment")?"<label><input type='checkbox' name='comments' value='1' class='jsonly' onclick=\"columnShow(this.checked, 6); toggle('Comment'); if (this.checked) this.form['Comment'].focus();\"".($sb?" checked":"").">".lang(97)."</label>".' <input name="Comment" id="Comment" value="'.h($K["Comment"]).'" maxlength="'.($h->server_info>=5.5?2048:60).'"'.($sb?'':' class="hidden"').'>':''),'<p>
...
';
    if ($a!="") {
        echo'<input type="submit" name="drop" value="',lang(117),'"',confirm(),'>';
    }
    if (support("partitioning")) {
        $mf=preg_match('~RANGE|LIST~', $K["partition_by"]);
        print_fieldset("partition", lang(165), $K["partition_by"]);
        echo'<p>
...
';
    ...
    if (preg_match('~MyISAM|M?aria'.($h->server_info>=5.6?'|InnoDB':'').'~i', $R["Engine"])) {
        $pd[]="FULLTEXT";
    }
    $v=indexes($a);
    $Bf=array();
    ...
    if ($_POST&&!$n&&!$_POST["add"]&&!$_POST["drop_col"]) {
        ...
        foreach ($K["indexes"]as$u) {
            $C=$u["name"]; if (in_array($u["type"], $pd)) {
                $f=array();
                $Wd=array();
                $Pb=array();
                $O=array();
                ...
                if ($f) {
                    $xc=$v[$C];
                    if ($xc) {
                        ksort($xc["columns"]);ksort($xc["lengths"]);ksort($xc["descs"]); if ($u["type"]==$xc["type"]&&array_values($xc["columns"])===$f&&(!$xc["lengths"]||array_values($xc["lengths"])===$Wd)&&array_values($xc["descs"])===$Pb) {
                            unset($v[$C]);
                            continue;
                        }
                    }
        ...
        foreach ($v
as$C=>$xc) {
            $c[]=array($xc["type"],$C,"DROP");
        }
        if (!$c) {
            redirect(ME."table=".urlencode($a));
        }
        queries_redirect(ME."table=".urlencode($a), lang(169), alter_indexes($a, $c));
    }
    page_header(lang(122), $n, array("table"=>$a), h($a));
    ...
    if ($_POST["add"]) {
        ...
        }$u=end($K["indexes"]); if ($u["type"]||array_filter($u["columns"], 'strlen')) {
...
';
    if ($Bf) {
        ...
        foreach ($Bf["columns"]as$x=>$e) {
            echo
select_input(" disabled", $p, $e),"<label><input disabled type='checkbox'>".lang(51)."</label> ";
        }
        echo"<td><td>\n";
    }
    $Fd=1;
    foreach ($K["indexes"]as$u) {
        if (!$_POST["drop_col"]||$Fd!=key($_POST["drop_col"])) {
            ...
            $s=1;
            foreach ($u["columns"]as$x=>$e) {
                echo"<span>".select_input(" name='indexes[$Fd][columns][$s]' onchange=\"".($s==count($u["columns"])?"indexesAddColumn":"indexesChangeColumn")."(this, '".h(js_escape($w=="sql"?"":$_GET["indexes"]."_"))."');\"", ($p?array_combine($p, $p):$p), $e),($w=="sql"||$w=="mssql"?"<input type='number' name='indexes[$Fd][lengths][$s]' class='size' value='".h($u["lengths"][$x])."'>":""),($w!="sql"?checkbox("indexes[$Fd][descs][$s]", 1, $u["descs"][$x], lang(51)):"")," </span>";
                $s++;
            }
...
';
    ...
    if ($_POST&&!$n&&!isset($_POST["add_x"])) {
        $C=trim($K["name"]); if ($_POST["drop"]) {
            $_GET["db"]="";
            queries_redirect(remove_from_uri("db|database"), lang(173), drop_databases(array(DB)));
        } elseif (DB!==$C) {
            if (DB!="") {
                $_GET["db"]=$C;
                queries_redirect(preg_replace('~\bdb=[^&]*&~', '', ME)."db=".urlencode($C), lang(174), rename_database($C, $K["collation"]));
            } else {
                $l=explode("\n", str_replace("\r", "", $C));
                $Ng=true;
                $Qd="";
...
as$m) {
                ...
                restart_session();
                set_session("dbs", null);
                queries_redirect(ME."db=".urlencode($Qd), lang(175), $Ng);
            }
        } else {
            if (!$K["collation"]) {
                redirect(substr(ME, 0, -1));
            }
            query_redirect("ALTER DATABASE ".idf_escape($C).(preg_match('~^[a-z0-9_]+$~i', $K["collation"])?" COLLATE $K[collation]":""), substr(ME, 0, -1), lang(176));
        }
    }
    page_header(DB!=""?lang(60):lang(177), $n, array(), h(DB));
    $nb=collations();
    $C=DB;
    if ($_POST) {
        $C=$K["name"];
    } elseif (DB!="") {
        $K["collation"]=db_collation(DB, $nb);
    } elseif ($w=="sql") {
...
';
    if (DB!="") {
        ...
        echo"<input type='image' class='icon' name='add' src='".h(preg_replace("~\\?.*~", "", ME))."?file=plus.gif&amp;version=4.2.5' alt='+' title='".lang(98)."'>\n";
    }
    echo'<input type="hidden" name="token" value="',$T,'">
</form>
';
    ...
    $K=$_POST;
    if ($_POST&&!$n) {
        $_=preg_replace('~ns=[^&]*&~', '', ME)."ns="; if ($_POST["drop"]) {
            query_redirect("DROP SCHEMA ".idf_escape($_GET["ns"]), $_, lang(178));
        } else {
            $C=trim($K["name"]);
            $_.=urlencode($C);
            if ($_GET["ns"]=="") {
                query_redirect("CREATE SCHEMA ".idf_escape($C), $_, lang(179));
            } elseif ($_GET["ns"]!=$C) {
                query_redirect("ALTER SCHEMA ".idf_escape($_GET["ns"])." RENAME TO ".idf_escape($C), $_, lang(180));
            } else {
                redirect($_);
            }
        }
    }
    page_header($_GET["ns"]!=""?lang(61):lang(62), $n);
    if (!$K) {
        $K["name"]=$_GET["ns"];
    }
    echo'
...
';
    ...
    $da=$_GET["call"];
    page_header(lang(181).": ".h($da), $n);
    $hg=routine($da, (isset($_GET["callf"])?"FUNCTION":"PROCEDURE"));
    $nd=array();
    $ff=array();
    foreach ($hg["fields"]as$s=>$o) {
        if (substr($o["inout"], -3)=="OUT") {
            $ff[$s]="@".idf_escape($o["field"])." AS ".idf_escape($o["field"]);
        } if (!$o["inout"]||substr($o["inout"], 0, 2)=="IN") {
            $nd[]=$s;
        }
    }
    if (!$n&&$_POST) {
        $Ya=array(); foreach ($hg["fields"]as$x=>$o) {
            if (in_array($x, $nd)) {
                $X=process_input($o); if ($X===false) {
                    $X="''";
                } if (isset($ff[$x])) {
                    $h->query("SET @".idf_escape($o["field"])." = $X");
                }
            }
            $Ya[]=(isset($ff[$x])?"@".idf_escape($o["field"]):$X);
        }$H=(isset($_GET["callf"])?"SELECT":"CALL")." ".idf_escape($da)."(".implode(", ", $Ya).")";echo"<p><code class='jush-$w'>".h($H)."</code> <a href='".h(ME)."sql=".urlencode($H)."'>".lang(10)."</a>\n"; if (!$h->multi_query($H)) {
            echo"<p class='error'>".error()."\n";
        } else {
            $i=connect();
            if (is_object($i)) {
            ...
            do {
                $I=$h->store_result(); if (is_object($I)) {
                    select($I, $i);
                } else {
            ...
            if ($ff) {
                select($h->query("SELECT ".implode(", ", $ff)));
            }
        }
    }
    echo'
...
';
    ...
    if ($_POST&&!$n&&!$_POST["add"]&&!$_POST["change"]&&!$_POST["change-js"]) {
        $le=($_POST["drop"]?lang(183):($C!=""?lang(184):lang(185)));$A=ME."table=".urlencode($a);$K["source"]=array_filter($K["source"], 'strlen');ksort($K["source"]);$bh=array(); foreach ($K["source"]as$x=>$X) {
            $bh[$x]=$K["target"][$x];
        }$K["target"]=$bh; if ($w=="sqlite") {
            queries_redirect($A, $le, recreate_table($a, $a, array(), array(), array(" $C"=>($_POST["drop"]?"":" ".format_foreign_key($K)))));
        } else {
            $c="ALTER TABLE ".table($a);
            $Yb="\nDROP ".($w=="sql"?"FOREIGN KEY ":"CONSTRAINT ").idf_escape($C);
            if ($_POST["drop"]) {
                query_redirect($c.$Yb, $A, $le);
            } else {
                query_redirect($c.($C!=""?"$Yb,":"")."\nADD".format_foreign_key($K), $A, $le);
                $n=lang(186)."<br>$n";
            }
        }
    }
    page_header(lang(187), $n, array("table"=>$a), h($a));
    if ($_POST) {
        ksort($K["source"]); if ($_POST["add"]) {
            $K["source"][]="";
    ...
    $Tf=array_keys(array_filter(table_status('', true), 'fk_support'));
    echo'
<form action="" method="post">
<p>
';
...
';
    ...
    $K=$_POST;
    if ($_POST&&!$n) {
        $C=trim($K["name"]);$Ga=" AS\n$K[select]";$A=ME."table=".urlencode($C);$le=lang(191); if ($_GET["materialized"]) {
            $U="MATERIALIZED VIEW";
        } else {
            $U="VIEW";
            if ($w=="pgsql") {
            ...
            query_redirect(($w=="mssql"?"ALTER":"CREATE OR REPLACE")." VIEW ".table($C).$Ga, $A, $le);
        } else {
            $dh=$C."_adminer_".uniqid();
            drop_create("DROP $U ".table($a), "CREATE $U ".table($C).$Ga, "DROP $U ".table($C), "CREATE $U ".table($dh).$Ga, "DROP $U ".table($dh), ($_POST["drop"]?substr(ME, 0, -1):$A), lang(192), $le, lang(193), $a, $C);
        }
    }
    if (!$_POST&&$a!="") {
        $K=view($a);$K["name"]=$a; if (!$n) {
            $n=error();
        }
    }
    page_header(($a!=""?lang(41):lang(194)), $n, array("table"=>$a), h($a));
    echo'
<form action="" method="post">
...
';
    ...
    if ($_POST&&!$n) {
        if ($_POST["drop"]) {
        ...
        } elseif (in_array($K["INTERVAL_FIELD"], $xd)&&isset($Jg[$K["STATUS"]])) {
            ...
            queries_redirect(substr(ME, 0, -1), ($aa!=""?lang(196):lang(197)), queries(($aa!=""?"ALTER EVENT ".idf_escape($aa).$mg.($aa!=$K["EVENT_NAME"]?"\nRENAME TO ".idf_escape($K["EVENT_NAME"]):""):"CREATE EVENT ".idf_escape($K["EVENT_NAME"]).$mg)."\n".$Jg[$K["STATUS"]]." COMMENT ".q($K["EVENT_COMMENT"]).rtrim(" DO\n$K[EVENT_DEFINITION]", ";").";"));
        }
    }
    page_header(($aa!=""?lang(198).": ".h($aa):lang(199)), $n);
    if (!$K&&$aa!="") {
...
';
    ...
    $da=$_GET["procedure"];
    $hg=(isset($_GET["function"])?"FUNCTION":"PROCEDURE");
    $K=$_POST;
    $K["fields"]=(array)$K["fields"];
    if ($_POST&&!process_fields($K["fields"])&&!$n) {
        $dh="$K[name]_adminer_".uniqid();
        drop_create("DROP $hg ".idf_escape($da), create_routine($hg, $K), "DROP $hg ".idf_escape($K["name"]), create_routine($hg, array("name"=>$dh)+$K), "DROP $hg ".idf_escape($dh), substr(ME, 0, -1), lang(204), lang(205), lang(206), $da, $K["name"]);
    }
    page_header(($da!=""?(isset($_GET["function"])?lang(207):lang(208)).": ".h($da):(isset($_GET["function"])?lang(209):lang(210))), $n);
    if (!$_POST&&$da!="") {
        $K=routine($da, $hg);
        $K["name"]=$da;
    }
...
';
    ...
    $K=$_POST;
    if ($_POST&&!$n) {
        $_=substr(ME, 0, -1);$C=trim($K["name"]); if ($_POST["drop"]) {
            query_redirect("DROP SEQUENCE ".idf_escape($fa), $_, lang(212));
        } elseif ($fa=="") {
            query_redirect("CREATE SEQUENCE ".idf_escape($C), $_, lang(213));
        } elseif ($fa!=$C) {
            query_redirect("ALTER SEQUENCE ".idf_escape($fa)." RENAME TO ".idf_escape($C), $_, lang(214));
        } else {
            redirect($_);
        }
    }
    page_header($fa!=""?lang(215).": ".h($fa):lang(216), $n);
    if (!$K) {
        $K["name"]=$fa;
    }
    echo'
...
';
    ...
    $K=$_POST;
    if ($_POST&&!$n) {
        $_=substr(ME, 0, -1); if ($_POST["drop"]) {
            query_redirect("DROP TYPE ".idf_escape($ga), $_, lang(217));
        } else {
            query_redirect("CREATE TYPE ".idf_escape(trim($K["name"]))." $K[as]", $_, lang(218));
        }
    }
    page_header($ga!=""?lang(219).": ".h($ga):lang(220), $n);
    if (!$K) {
        $K["as"]="AS ";
    }
    echo'
...
';
    ...
    if ($_POST) {
        if (!$n&&in_array($_POST["Timing"], $yh["Timing"])&&in_array($_POST["Event"], $yh["Event"])&&in_array($_POST["Type"], $yh["Type"])) {
            $Le=" ON ".table($a);$Yb="DROP TRIGGER ".idf_escape($C).($w=="pgsql"?$Le:"");$A=ME."table=".urlencode($a); if ($_POST["drop"]) {
                query_redirect($Yb, $A, lang(221));
            } else {
                if ($C!="") {
                    queries($Yb);
                }
                queries_redirect($A, ($C!=""?lang(222):lang(223)), queries(create_trigger($Le, $_POST)));
                if ($C!="") {
                    queries(create_trigger($Le, $K+array("Type"=>reset($yh["Type"]))));
                }
            }
        }
        $K=$_POST;
    }
    page_header(($C!=""?lang(224).": ".h($C):lang(225)), $n, array("table"=>$a));
    echo'
<form action="" method="post" id="form">
<table cellspacing="0">
...
';
    ...
    foreach (get_rows("SHOW PRIVILEGES")as$K) {
        foreach (explode(",", ($K["Privilege"]=="Grant option"?"":$K["Context"]))as$yb) {
            $Gf[$yb][$K["Privilege"]]=$K["Comment"];
        }
    }
    ...
    foreach ($Gf["Tables"]as$x=>$X) {
        unset($Gf["Databases"][$x]);
    }
    $ye=array();
    if ($_POST) {
        foreach ($_POST["objects"]as$x=>$X) {
            $ye[$X]=(array)$ye[$X]+(array)$_POST["grants"][$x];
        }
    }
    $Wc=array();
    $Je="";
...
as$X) {
    ...
    if ($_POST&&!$n) {
        $Ke=(isset($_GET["host"])?q($ha)."@".q($_GET["host"]):"''"); if ($_POST["drop"]) {
            query_redirect("DROP USER $Ke", ME."privileges=", lang(228));
        } else {
            $_e=q($_POST["user"])."@".q($_POST["host"]);
            $pf=$_POST["pass"];
            if ($pf!=''&&!$_POST["hashed"]) {
                $pf=$h->result("SELECT PASSWORD(".q($pf).")");
                $n=!$pf;
            }
            $Cb=false;
            if (!$n) {
                if ($Ke!=$_e) {
                    $Cb=queries(($h->server_info<5?"GRANT USAGE ON *.* TO":"CREATE USER")." $_e IDENTIFIED BY PASSWORD ".q($pf));
                    $n=!$Cb;
                } elseif ($pf!=$Je) {
                    queries("SET PASSWORD FOR $_e = ".q($pf));
                }
            }
            if (!$n) {
                $eg=array(); foreach ($ye
as$Ee=>$Vc) {
                    ...
                    $Vc=array_keys($Vc);
                    if (isset($_GET["grant"])) {
                        $eg=array_diff(array_keys(array_filter($ye[$Ee], 'strlen')), $Vc);
                    } elseif ($Ke==$_e) {
                        $He=array_keys((array)$Wc[$Ee]);
                        $eg=array_diff($He, $Vc);
                        $Vc=array_diff($Vc, $He);
                        unset($Wc[$Ee]);
                    }
                    if (preg_match('~^(.+)\\s*(\\(.*\\))?$~U', $Ee, $B)&&(!grant("REVOKE", $eg, $B[2], " ON $B[1] FROM $_e")||!grant("GRANT", $Vc, $B[2], " ON $B[1] TO $_e"))) {
                        $n=true;
                        break;
                    }
                }
            ...
            if (!$n&&isset($_GET["host"])) {
                if ($Ke!=$_e) {
                    ...
                    foreach ($Wc
as$Ee=>$eg) {
                        if (preg_match('~^(.+)(\\(.*\\))?$~U', $Ee, $B)) {
                            grant("REVOKE", array_keys($eg), $B[2], " ON $B[1] FROM $_e");
                        }
                    }
                }
            }
            queries_redirect(ME."privileges=", (isset($_GET["host"])?lang(229):lang(230)), !$n);
            if ($Cb) {
                $h->query("DROP USER $_e");
            }
        }
    }
    page_header((isset($_GET["host"])?lang(33).": ".h("$ha@$_GET[host]"):lang(136)), $n, array("privileges"=>array('',lang(64))));
    if ($_POST) {
        $K=$_POST;
        $Wc=$ye;
    } else {
...
';
    echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'>".lang(64).doc_link(array('sql'=>"grant.html#priv_level"));
    $s=0;
    foreach ($Wc
as$Ee=>$Vc) {
        echo'<th>'.($Ee!="*.*"?"<input name='objects[$s]' value='".h($Ee)."' size='10' autocapitalize='off'>":"<input type='hidden' name='objects[$s]' value='*.*' size='10'>*.*");
        $s++;
    }
    echo"</thead>\n";
    foreach (array(""=>"","Server Admin"=>lang(32),"Databases"=>lang(35),"Tables"=>lang(120),"Columns"=>lang(121),"Procedures"=>lang(232),)as$yb=>$Ob) {
        foreach ((array)$Gf[$yb]as$Ff=>$rb) {
            echo"<tr".odd()."><td".($Ob?">$Ob<td":" colspan='2'").' lang="en" title="'.h($rb).'">'.h($Ff);
            $s=0;
            foreach ($Wc
as$Ee=>$Vc) {
                $C="'grants[$s][".h(strtoupper($Ff))."]'";
                $Y=$Vc[strtoupper($Ff)];
                if ($yb=="Server Admin"&&$Ee!=(isset($Wc["*.*"])?"*.*":".*")) {
                    echo"<td>&nbsp;";
                } elseif (isset($_GET["grant"])) {
                    echo"<td><select name=$C><option><option value='1'".($Y?" selected":"").">".lang(233)."<option value='0'".($Y=="0"?" selected":"").">".lang(234)."</select>";
                } else {
                    echo"<td align='center'><label class='block'><input type='checkbox' name=$C value='1'".($Y?" checked":"").($Ff=="All privileges"?" id='grants-$s-all'":($Ff=="Grant option"?"":" onclick=\"if (this.checked) formUncheck('grants-$s-all');\""))."></label>";
                }
                $s++;
            }
        }
...
';
    ...
    if (support("kill")&&$_POST&&!$n) {
        ...
        queries_redirect(ME."processlist=", lang(235, $Md), $Md||!$_POST["kill"]);
    }
    page_header(lang(106), $n);
    echo'
<form action="" method="post">
...
';
    ...
    foreach (process_list()as$s=>$K) {
        if (!$s) {
            ...
            foreach ($K
as$x=>$X) {
                echo"<th>$x".doc_link(array('sql'=>"show-processlist.html#processlist_".strtolower($x),'pgsql'=>"monitoring-stats.html#PG-STAT-ACTIVITY-VIEW",'oracle'=>"../b14237/dynviews_2088.htm",));
            }
            echo"</thead>\n";
        }
        ...
        foreach ($K
as$x=>$X) {
            echo"<td>".(($w=="sql"&&$x=="Info"&&preg_match("~Query|Killed~", $K["Command"])&&$X!="")||($w=="pgsql"&&$x=="current_query"&&$X!="<IDLE>")||($w=="oracle"&&$x=="sql_text"&&$X!="")?"<code class='jush-$w'>".shorten_utf8($X, 100, "</code>").' <a href="'.h(ME.($K["db"]!=""?"db=".urlencode($K["db"])."&":"")."sql=".urlencode($X)).'">'.lang(236).'</a>':nbsp($X));
        }
        echo"\n";
    }
    echo'</table>
...
';
    if (support("kill")) {
        echo($s+1)."/".lang(237, max_connections()),"<p><input type='submit' value='".lang(238)."'>\n";
    }
    echo'<input type="hidden" name="token" value="',$T,'">
</form>
';
} elseif (isset($_GET["select"])) {
    $a=$_GET["select"];$R=table_status1($a);$v=indexes($a);$p=fields($a);$Oc=column_foreign_keys($a);$Ge=""; if ($R["Oid"]) {
        $Ge=($w=="sqlite"?"rowid":"oid");
        $v[]=array("type"=>"PRIMARY","columns"=>array($Ge));
    }parse_str($_COOKIE["adminer_import"], $ya);$fg=array();$f=array();$hh=null; foreach ($p
as$x=>$o) {
        $C=$b->fieldName($o);
        if (isset($o["privileges"]["select"])&&$C!="") {
            $f[$x]=html_entity_decode(strip_tags($C), ENT_QUOTES); if (is_shortable($o)) {
                $hh=$b->selectLengthProcess();
            }
        }
        $fg+=$o["privileges"];
    }list($M, $Xc)=$b->selectColumnsProcess($f, $v);$Ad=count($Xc)<count($M);$Z=$b->selectSearchProcess($p, $v);$We=$b->selectOrderProcess($p, $v);$z=$b->selectLimitProcess();$Tc=($M?implode(", ", $M):"*".($Ge?", $Ge":"")).convert_fields($f, $p, $M)."\nFROM ".table($a);$Yc=($Xc&&$Ad?"\nGROUP BY ".implode(", ", $Xc):"").($We?"\nORDER BY ".implode(", ", $We):""); if ($_GET["val"]&&is_ajax()) {
        header("Content-Type: text/plain; charset=utf-8");
        foreach ($_GET["val"]as$Fh=>$K) {
            $Ga=convert_field($p[key($K)]);$M=array($Ga?$Ga:idf_escape(key($K)));$Z[]=where_check($Fh, $p);$J=$Wb->select($a, $M, $Z, $M); if ($J) {
                echo
reset($J->fetch_row());
            }
        }
...
as$x=>$X) {
        ...
        if ($_POST["export"]) {
            ...
            dump_headers($a);
            $b->dumpTable($a, "");
            if (!is_array($_POST["check"])||$Hh===array()) {
                $H="SELECT $Tc$ci$Yc";
            } else {
                $Dh=array();
...
as$C=>$X) {
                ...
                queries_redirect(remove_from_uri($_POST["all"]&&$_POST["delete"]?"page":""), $le, $I);
                if (!$_POST["delete"]) {
...
as$x=>$X) {
                    ...
                    queries_redirect(remove_from_uri(), lang(239, $za), $I);
                }
                ...
                $za=count($de[0]);
                $Wb->begin();
                $ug=($_POST["separator"]=="csv"?",":($_POST["separator"]=="tsv"?"\t":";"));
                $L=array();
                foreach ($de[0]as$x=>$X) {
                    preg_match_all("~((?>\"[^\"]*\")+|[^$ug]*)$ug~", $X.$ug, $ee); if (!$x&&!array_diff($ee[1], $ob)) {
                        $ob=$ee[1];
                        $za--;
                    } else {
                        $O=array();
                        foreach ($ee[1]as$s=>$lb) {
                            $O[idf_escape($ob[$s])]=($lb==""&&$p[$ob[$s]]["null"]?"NULL":q(str_replace('""', '"', preg_replace('~^"|"$~', '', $lb))));
                        }
                        $L[]=$O;
                    }
                }
                $I=(!$L||$Wb->insertUpdate($a, $L, $Bf));
                if ($I) {
                    $Wb->commit();
                }
                queries_redirect(remove_from_uri("page"), lang(242, $za), $I);
                $Wb->rollback();
            }
        }
    }$Tg=$b->tableName($R); if (is_ajax()) {
        page_headers();
        ob_start();
    } else {
        page_header(lang(45).": $Tg", $n);
    }$O=null; if (isset($fg["insert"])||!support("table")) {
        $O="";
        foreach ((array)$_GET["where"]as$X) {
            if (count($Oc[$X["col"]])==1&&($X["op"]=="="||(!$X["op"]&&!preg_match('~[_%]~', $X["val"])))) {
                $O.="&set".urlencode("[".bracket_escape($X["col"])."]")."=".urlencode($X["val"]);
            }
        }
    }$b->selectLinks($R, $O); if (!$f&&support("table")) {
        echo"<p class='error'>".lang(243).($p?".":": ".error())."\n";
    } else {
        ...
        $I=$Wb->select($a, $rg, $Z, $Xc, $We, $z, $E, true);
        if (!$I) {
            echo"<p class='error'>".error()."\n";
        } else {
            if ($w=="mssql"&&$E) {
            ...
            if ($_GET["page"]!="last"&&+$z&&$Xc&&$Ad&&$w=="sql") {
                $Rc=$h->result(" SELECT FOUND_ROWS()");
            }
            if (!$L) {
                echo"<p class='message'>".lang(12)."\n";
            } else {
                $Pa=$b->backwardKeys($a, $Tg);
                echo"<table id='table' cellspacing='0' class='nowrap checkable' onclick='tableClick(event);' ondblclick='tableClick(event, true);' onkeydown='return editingKeydown(event);'>\n","<thead><tr>".(!$Xc&&$M?"":"<td><input type='checkbox' id='all-page' onclick='formCheck(this, /check/);'> <a href='".h($_GET["modify"]?remove_from_uri("modify"):$_SERVER["REQUEST_URI"]."&modify=1")."'>".lang(244)."</a>");
                $xe=array();
                $Uc=array();
                reset($M);
                $Pf=1;
                foreach ($L[0]as$x=>$X) {
                    if ($x!=$Ge) {
                        ...
                        if ($C!="") {
                            ...
                            $Ob="&desc%5B0%5D=1";
                            echo'<th onmouseover="columnMouse(this);" onmouseout="columnMouse(this, \' hidden\');">','<a href="'.h($id.($We[0]==$e||$We[0]==$x||(!$We&&$Ad&&$Xc[0]==$e)?$Ob:'')).'">';
                            echo
apply_sql_function($X["fun"], $C)."</a>";
                            echo"<span class='column hidden'>","<a href='".h($id.$Ob)."' title='".lang(51)."' class='text'> â†“</a>";
                            if (!$X["fun"]) {
                                echo'<a href="#fieldset-search" onclick="selectSearch(\''.h(js_escape($x)).'\'); return false;" title="'.lang(48).'" class="text jsonly"> =</a>';
                            }
                            echo"</span>";
                        }
                        $Uc[$x]=$X["fun"];
...
as$x=>$X) {
                ...
                foreach ($b->rowDescriptions($L, $Oc)as$we=>$K) {
                    $Eh=unique_array($L[$we], $v);
                    if (!$Eh) {
                        $Eh=array(); foreach ($L[$we]as$x=>$X) {
...
as$x=>$X) {
                    ...
                    echo"<tr".odd().">".(!$Xc&&$M?"":"<td>".checkbox("check[]", substr($Fh, 1), in_array(substr($Fh, 1), (array)$_POST["check"]), "", "this.form['all'].checked = false; formUncheck('all-page');").($Ad||information_schema(DB)?"":" <a href='".h(ME."edit=".urlencode($a).$Fh)."'>".lang(246)."</a>"));
                    foreach ($K
as$x=>$X) {
                        if (isset($xe[$x])) {
                            $o=$p[$x];
                            if ($X!=""&&(!isset($lc[$x])||$lc[$x]!="")) {
                                $lc[$x]=(is_mail($X)?$xe[$x]:"");
                            }
                            $_="";
                            if (preg_match('~blob|bytea|raw|file~', $o["type"])&&$X!="") {
                                $_=ME.'download='.urlencode($a).'&field='.urlencode($x).$Fh;
                            }
                            if (!$_&&$X!==null) {
                                foreach ((array)$Oc[$x]as$q) {
                                    if (count($Oc[$x])==1||end($q["source"])==$x) {
                                        $_=""; foreach ($q["source"]as$s=>$Dg) {
                                            $_.=where_link($s, $q["target"][$s], $L[$we][$Dg]);
                                        }$_=($q["db"]!=""?preg_replace('~([?&]db=)[^&]+~', '\\1'.urlencode($q["db"]), ME):ME).'select='.urlencode($q["table"]).$_; if (count($q["source"])==1) {
                                            break;
                                        }
                                    }
                                }
                            ...
                            if ($x=="COUNT(*)") {
                                $_=ME."select=".urlencode($a);$s=0; foreach ((array)$_GET["where"]as$W) {
                                    if (!array_key_exists($W["col"], $Eh)) {
...
as$Gd=>$W) {
                            ...
                            $gc=!is_array($K[$x])&&is_utf8($X)&&$L[$we][$x]==$K[$x]&&!$Uc[$x];
                            ...
                            if (($_GET["modify"]&&$gc)||$Y!==null) {
                                ...
                                $ae=strpos($X, "<i>...</i>");
                                echo"<td id='$jd' onclick=\"selectClick(this, event, ".($ae?2:($gh?1:0)).($gc?"":", '".h(lang(247))."'").");\">$X";
                            }
                        }
                    }
                    if ($Pa) {
            ...
            if (($L||$E)&&!is_ajax()) {
                ...
                if ($_GET["page"]!="last") {
                    if (!+$z) {
                        ...
                        if ($Rc<max(1e4, 2*($E+1)*$z)) {
                            $Rc=reset(slow_query(count_rows($a, $Z, $Ad, $Xc)));
                        } else {
                            $vc=false;
                        }
                    }
                ...
                if (+$z&&($Rc===false||$Rc>$z||$E)) {
                    echo"<p class='pages'>";$ge=($Rc===false?$E+(count($L)>=$z?2:1):floor(($Rc-1)/$z)); if ($w!="simpledb") {
                        echo'<a href="'.h(remove_from_uri("page"))."\" onclick=\"pageClick(this.href, +prompt('".lang(248)."', '".($E+1)."'), event); return false;\">".lang(248)."</a>:",pagination(0, $E).($E>5?" ...":"");
                        for ($s=max(1, $E-4);$s<min($ge, $E+5);$s++) {
                            echo
pagination($s, $E);
                        }
                        if ($ge>0) {
                            echo($E+5<$ge?" ...":""),($vc&&$Rc!==false?pagination($ge, $E):" <a href='".h(remove_from_uri("page")."&page=last")."' title='~$ge'>".lang(249)."</a>");
                        }
                        echo(($Rc===false?count($L)+1:$Rc-$E*$z)>$z?' <a href="'.h(remove_from_uri("page")."&page=".($E+1)).'" onclick="return !selectLoadMore(this, '.(+$z).', \''.lang(250).'...\');" class="loadmore">'.lang(251).'</a>':'');
                    } else {
                        echo
lang(248).":",pagination(0, $E).($E>1?" ...":""),($E?pagination($E, $E):""),($ge>$E?pagination($E+1, $E).($ge>$E+1?" ...":""):"");
                    }
                }
                echo"<p class='count'>\n",($Rc!==false?"(".($vc?"":"~ ").lang(140, $Rc).") ":"");
                $Tb=($vc?"":"~ ").$Rc;
                echo
checkbox("all", 1, 0, lang(252), "var checked = formChecked(this, /check/); selectCount('selected', this.checked ? '$Tb' : checked); selectCount('selected2', this.checked || !checked ? '$Tb' : checked);")."\n";
                if ($b->selectCommandPrint()) {
                    echo'<fieldset',($_GET["modify"]?'':' class="jsonly"'),'><legend>',lang(244),'</legend><div>
...
';
                ...
                if ($Pc) {
                    ...
                    $gf=$b->dumpOutput();
                    echo($gf?html_select("output", $gf, $ya["output"])." ":""),html_select("format", $Pc, $ya["format"])," <input type='submit' name='export' value='".lang(66)."'>\n","</div></fieldset>\n";
                }
                echo(!$Xc&&$M?"":"<script type='text/javascript'>tableCheck();</script>\n");
            }
            if ($b->selectImportPrint()) {
                print_fieldset("import", lang(65), !$L);
    ...
    $Ig=isset($_GET["status"]);page_header($Ig?lang(108):lang(107));$Uh=($Ig?show_status():show_variables()); if (!$Uh) {
        echo"<p class='message'>".lang(12)."\n";
    } else {
        echo"<table cellspacing='0'>\n";
        foreach ($Uh
as$x=>$X) {
    ...
    if ($_GET["script"]=="db") {
        ...
        foreach (table_status()as$C=>$R) {
            json_row("Comment-$C", nbsp($R["Comment"])); if (!is_view($R)) {
                foreach (array("Engine","Collation")as$x) {
                ...
                foreach ($Qg+array("Auto_increment"=>0,"Rows"=>0)as$x=>$X) {
                    if ($R[$x]!="") {
                        ...
                        if (isset($Qg[$x])) {
                            $Qg[$x]+=($R["Engine"]!="InnoDB"||$x!="Data_free"?$R[$x]:0);
                        }
...
as$x=>$X) {
    ...
    $Zg=array_merge((array)$_POST["tables"], (array)$_POST["views"]); if ($Zg&&!$n&&!$_POST["search"]) {
        $I=true;
        $le="";
        ...
        if ($_POST["truncate"]) {
            ...
            while ($K=$I->fetch_assoc()) {
                $le.="<b>".h($K["Table"])."</b>: ".h($K["Msg_text"])."<br>";
            }
        }
        queries_redirect(substr(ME, 0, -1), $le, $I);
    }page_header(($_GET["ns"]==""?lang(35).": ".h(DB):lang(69).": ".h($_GET["ns"])), $n, true); if ($b->homepage()) {
        if ($_GET["ns"]!=="") {
            echo"<h3 id='tables-views'>".lang(258)."</h3>\n";$Yg=tables_list(); if (!$Yg) {
                echo"<p class='message'>".lang(9)."\n";
            } else {
                echo"<form action='' method='post'>\n";
                if (support("table")) {
                    echo"<fieldset><legend>".lang(259)." <span id='selected2'></span></legend><div>","<input type='search' name='query' value='".h($_POST["query"])."'> <input type='submit' name='search' value='".lang(48)."'>\n","</div></fieldset>\n"; if ($_POST["search"]&&$_POST["query"]!="") {
                        search_tables();
                    }
                }
                ...
                echo'<th>'.lang(120),'<td>'.lang(260).doc_link(array('sql'=>'storage-engines.html')),'<td>'.lang(112).doc_link(array('sql'=>'charset-mysql.html')),'<td>'.lang(261).$Ub,'<td>'.lang(262).$Ub,'<td>'.lang(263).$Ub,'<td>'.lang(57).doc_link(array('sql'=>'example-auto-increment.html')),'<td>'.lang(264).$Ub,(support("comment")?'<td>'.lang(97).$Ub:''),"</thead>\n";
                $S=0;
                foreach ($Yg
as$C=>$U) {
                    $Xh=($U!==null&&!preg_match('~table~i', $U));
                    echo'<tr'.odd().'><td>'.checkbox(($Xh?"views[]":"tables[]"), $C, in_array($C, $Zg, true), "", "formUncheck('check-all');"),'<th>'.(support("table")||support("indexes")?'<a href="'.h(ME).'table='.urlencode($C).'" title="'.lang(40).'">'.h($C).'</a>':h($C));
                    if ($Xh) {
                        echo'<td colspan="6"><a href="'.h(ME)."view=".urlencode($C).'" title="'.lang(41).'">'.(preg_match('~materialized~i', $U)?lang(265):lang(119)).'</a>','<td align="right"><a href="'.h(ME)."select=".urlencode($C).'" title="'.lang(39).'">?</a>';
                    } else {
                        foreach (array("Engine"=>array(),"Collation"=>array(),"Data_length"=>array("create",lang(42)),"Index_length"=>array("indexes",lang(123)),"Data_free"=>array("edit",lang(43)),"Auto_increment"=>array("auto_increment=1&create",lang(42)),"Rows"=>array("select",lang(39)),)as$x=>$_) {
                            $jd=" id='$x-".h($C)."'";
                ...
                echo"<tr><td>&nbsp;<th>".lang(237, count($Yg)),"<td>".nbsp($w=="sql"?$h->result("SELECT @@storage_engine"):""),"<td>".nbsp(db_collation(DB, collations()));
                foreach (array("Data_length","Index_length","Data_free")as$x) {
                    echo"<td align='right' id='sum-$x'>&nbsp;";
                }
                echo"</table>\n";
                if (!information_schema(DB)) {
                    $Rh="<input type='submit' value='".lang(266)."'".on_help("'VACUUM'")."> ";
                    $Se="<input type='submit' name='optimize' value='".lang(267)."'".on_help($w=="sql"?"'OPTIMIZE TABLE'":"'VACUUM OPTIMIZE'")."> ";
                    echo"<fieldset><legend>".lang(116)." <span id='selected'></span></legend><div>".($w=="sqlite"?$Rh:($w=="pgsql"?$Rh.$Se:($w=="sql"?"<input type='submit' value='".lang(268)."'".on_help("'ANALYZE TABLE'")."> ".$Se."<input type='submit' name='check' value='".lang(269)."'".on_help("'CHECK TABLE'")."> "."<input type='submit' name='repair' value='".lang(270)."'".on_help("'REPAIR TABLE'")."> ":"")))."<input type='submit' name='truncate' value='".lang(271)."'".confirm().on_help($w=="sqlite"?"'DELETE'":"'TRUNCATE".($w=="pgsql"?"'":" TABLE'"))."> "."<input type='submit' name='drop' value='".lang(117)."'".confirm().on_help("'DROP TABLE'").">\n";
                    $l=(support("scheme")?$b->schemas():$b->databases());
                    if (count($l)!=1&&$w!="sqlite") {
                        $m=(isset($_POST["target"])?$_POST["target"]:(support("scheme")?$_GET["ns"]:DB));
                        echo"<p>".lang(272).": ",($l?html_select("target", $l, $m):'<input name="target" value="'.h($m).'" autocapitalize="off">')," <input type='submit' name='move' value='".lang(273)."'>",(support("copy")?" <input type='submit' name='copy' value='".lang(274)."'>":""),"\n";
                    }
                ...
                echo"<h3 id='routines'>".lang(133)."</h3>\n";
                $jg=routines();
                if ($jg) {
                    ...
                    foreach ($jg
as$K) {
                        echo'<tr'.odd().'>','<th><a href="'.h(ME).($K["ROUTINE_TYPE"]!="PROCEDURE"?'callf=':'call=').urlencode($K["ROUTINE_NAME"]).'">'.h($K["ROUTINE_NAME"]).'</a>','<td>'.h($K["ROUTINE_TYPE"]),'<td>'.h($K["DTD_IDENTIFIER"]),'<td><a href="'.h(ME).($K["ROUTINE_TYPE"]!="PROCEDURE"?'function=':'procedure=').urlencode($K["ROUTINE_NAME"]).'">'.lang(126)."</a>";
                    }
                    echo"</table>\n";
                }
                echo'<p class="links">'.(support("procedure")?'<a href="'.h(ME).'procedure=">'.lang(210).'</a>':'').'<a href="'.h(ME).'function=">'.lang(209)."</a>\n";
            } if (support("sequence")) {
                echo"<h3 id='sequences'>".lang(276)."</h3>\n";
                $vg=get_vals("SELECT sequence_name FROM information_schema.sequences WHERE sequence_schema = current_schema() ORDER BY sequence_name");
                if ($vg) {
...
as$X) {
                ...
                echo"<h3 id='events'>".lang(134)."</h3>\n";
                $L=get_rows("SHOW EVENTS");
                if ($L) {
                    echo"<table cellspacing='0'>\n","<thead><tr><th>".lang(172)."<td>".lang(277)."<td>".lang(200)."<td>".lang(201)."<td></thead>\n"; foreach ($L
as$K) {
                        echo"<tr>","<th>".h($K["Name"]),"<td>".($K["Execute at"]?lang(278)."<td>".$K["Execute at"]:lang(202)." ".$K["Interval value"]." ".$K["Interval field"]."<td>$K[Starts]"),"<td>$K[Ends]",'<td><a href="'.h(ME).'event='.urlencode($K["Name"]).'">'.lang(126).'</a>';
                    }echo"</table>\n";$tc=$h->result("SELECT @@event_scheduler"); if ($tc&&$tc!="ON") {
                        echo"<p class='error'><code class='jush-sqlset'>event_scheduler</code>: ".h($tc)."\n";
                    }
                }
